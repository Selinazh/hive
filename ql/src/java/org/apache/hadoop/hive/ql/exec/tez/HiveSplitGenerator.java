  ...
  public List<Event> initialize() throws Exception {
    ...
    try {
      ...
      if (groupingEnabled) {
        ...
        QueueHeadroomCalculator headroomCalculator =
            (QueueHeadroomCalculator)
                ReflectionUtils.newInstance(
                    JavaUtils.loadClass(
                        HiveConf.getVar(jobConf, HiveConf.ConfVars.HIVE_AM_SPLIT_GENERATION_HEADROOM_CALCULATOR)
                    ),
                    jobConf
                );

        int availableSlots = headroomCalculator.getAvailableSlots(getContext());
