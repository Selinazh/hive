...
class SnappyCodec implements CompressionCodec, DirectDecompressionCodec {
  ...
  public boolean compress(ByteBuffer in, ChannelBuffer out) throws IOException {
    ...
    int outBytes =
        Snappy.compress(in.array(), in.arrayOffset() + in.position(), inBytes,
            compressed, 0);
    if (outBytes < inBytes) {
      out.ensureWritableBytes(outBytes);
      System.arraycopy(compressed, 0, out.array(), out.arrayOffset() +
  ...
  public void decompress(ByteBuffer in, ChannelBuffer out, int maxBufferSize) throws IOException {
    int inOffset = in.position();
    int len = Snappy.getUncompressedLength(in.array(), in.arrayOffset() + inOffset);
    out.ensureWritableBytes(len);
    int uncompressLen =
        Snappy.uncompress(in.array(), in.arrayOffset() + inOffset,
        in.limit() - inOffset, out.array(), out.arrayOffset() + out.writerIndex());
    out.writerIndex(uncompressLen + out.writerIndex());
  }
}
