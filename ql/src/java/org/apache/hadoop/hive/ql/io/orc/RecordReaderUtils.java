...
public class RecordReaderUtils {
  ...
  static void addRgFilteredStreamToRanges(OrcProto.Stream stream,
    ...
    for (int group = 0; group < includedRowGroups.length; ++group) {
      ...
      if (firstGroup) {
        // skip merge the last empty row group in previous stream:
        // e.g col1, col2 DATA stream are adjacent, query include rw1, rw2 and rw4,
        // col1 DATA stream rw1 [a,b], rw2 [b,b), rw3 [b,c), rw4 [c,c)
        // col2 DATA stream rw2 [c,d)...
        // expecting result: col1 [a,b), [c,c) col2 [c,d)...
        // [c,c) need to keep to allow seek to empty row group
        list.addOrMerge(start, end, doMergeBuffers, true, true);
        firstGroup = false;
      } else {
        list.addOrMerge(start, end, doMergeBuffers, true, false);
      }
    }
  }
  ...
  static List<DiskRange> getStreamBuffers(DiskRangeList range, long offset, long length) {
    ...
    while (range != null) {
      ...
      } else if (range.getOffset() == range.getEnd() && range.getOffset() == streamEnd) {
        // Last empty row group in current stream
