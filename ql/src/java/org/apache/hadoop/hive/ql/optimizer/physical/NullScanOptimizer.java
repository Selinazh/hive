...
import org.apache.hadoop.hive.ql.optimizer.MetadataOnlyOptimizer.WalkerCtx;
...
public class NullScanOptimizer implements PhysicalPlanResolver {
  ...
  public PhysicalContext resolve(PhysicalContext pctx) throws SemanticException {
    ...
    opRules.put(new RuleRegExp("R1", TableScanOperator.getOperatorName()+ "%"),new TSMetadataOnlyMarker());
    disp = new MetadataOnlyTaskDispatcher(pctx, opRules);
    ogw = new DefaultGraphWalker(disp);
    topNodes = new ArrayList<Node>();
    topNodes.addAll(pctx.getRootTasks());
    ogw.startWalking(topNodes, null);
  ...
  static private class TSMetadataOnlyMarker implements NodeProcessor {

    @Override
    public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx,
        Object... nodeOutputs) throws SemanticException {
      if (((TableScanOperator)nd).getIsMetadataOnly()){
        ((WalkerCtx)procCtx).setMayBeMetadataOnly((TableScanOperator)nd);
        ((WalkerCtx)procCtx).convertMetadataOnly();
      }
      return null;
