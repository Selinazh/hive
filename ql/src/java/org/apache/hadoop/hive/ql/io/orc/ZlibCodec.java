...
class ZlibCodec implements CompressionCodec, DirectDecompressionCodec {
  ...
  @Override
  public boolean compress(ByteBuffer in, ChannelBuffer out) throws IOException {
    ...
    while (!deflater.finished() && (length > outSize)) {
      out.ensureWritableBytes(length);
  ...
  /**
   * @deprecated Use {@link #compress(ByteBuffer, ChannelBuffer)} instead.
   */
  @Deprecated
  @Override
  ...
  @Override
  public void decompress(ByteBuffer in, ChannelBuffer out, int maxBufferSize) throws IOException {
    Inflater inflater = new Inflater(true);
    int compressedLength = in.remaining();
    inflater.setInput(in.array(), in.arrayOffset() + in.position(), compressedLength);
    out.ensureWritableBytes(Math.min(in.remaining() << 2, maxBufferSize - out.readableBytes()));
    ...
    while (!inflater.finished() && !inflater.needsDictionary()) { 
      try {
        int count = inflater.inflate(out.array(),
                                     ...
                                     out.writableBytes());
        out.writerIndex(count + out.writerIndex());
        if (inflater.needsInput()) {
          inflater.setInput(in.array(), compressedLength - inflater.getRemaining(), inflater.getRemaining());
        } 
        if (!inflater.finished()) {
          out.ensureWritableBytes(Math.min(in.remaining() << 2, maxBufferSize));
        }
      } catch (DataFormatException dfe) {
        throw new IOException("Bad compression data", dfe);
      }
