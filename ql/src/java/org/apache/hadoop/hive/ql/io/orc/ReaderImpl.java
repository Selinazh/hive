...
public class ReaderImpl implements Reader {
  ...
  protected final FSDataInputStream file;
  ...
  private  int getRootColumn() {
    return hasMetadataValue(OrcRecordUpdater.ACID_KEY_INDEX_NAME) ? (OrcRecordUpdater.ROW + 1) : 0;
  }

  // filter stripes using stripeStatistics, in case it has not been done (only ETL split calculation
  // trim stripes beforehand)
  private List<StripeInformation> filterStripes(List<StripeInformation> stripes,Options options) {
    SearchArgument sarg = options.getSearchArgument();
    List<OrcProto.StripeStatistics> stripeStatistics = getOrcProtoStripeStatistics();
    if (sarg == null || stripeStatistics.size() < stripes.size()) {
      return stripes;
    }

    List<StripeInformation> filtered = new ArrayList<StripeInformation>();
    int[] columns =  RecordReaderImpl.mapSargColumns(sarg.getLeaves(), options.getColumnNames(), getRootColumn());
    for (int i = 0; i < stripes.size(); i++) {
      OrcProto.StripeStatistics stats = stripeStatistics.get(i);
      if (stats == null) {
        // in case some statistics are missing. should not happen
        filtered = stripes;
        break;
      }
      if (isStripeSatisfyPredicate(stats, sarg, columns)) {
        filtered.add(stripes.get(i));
      }
    }
    LOG.info("Total stripes needs to peek:" + filtered.size());
    return filtered;
  }

  // copied from OrcInputFormat.SplitGenerator, cannot reuse the code because the support classes
  // are different.
  private static boolean isStripeSatisfyPredicate(OrcProto.StripeStatistics stripeStatistics,
          SearchArgument sarg,
          int[] filterColumns) {
    ...
    for (int pred = 0; pred < truthValues.length; pred++) {
      if (filterColumns[pred] != -1) {
        ...
        OrcProto.ColumnStatistics stats = stripeStatistics.getColStats(filterColumns[pred]);
        truthValues[pred] = RecordReaderImpl.evaluatePredicateProto(stats, predLeaves.get(pred), null);
      } else {
  ...
  * Constructor that let's the user specify additional options.
  ...
  public ReaderImpl(Path path, OrcFile.ReaderOptions options) throws IOException {
    ...
    if (options.getFileMetaInfo() != null) {
      ...
      footerMetaData = extractMetaInfoFromFooter(fs, path, file,
  ...
  public RecordReader rowsOptions(Options options) throws IOException {
    ...
    List<StripeInformation> stripes = getStripes();
    return new RecordReaderImpl(stripes, filterStripes(stripes, options), fileSystem, path,
        options, footer.getTypesList(), codec, bufferSize,
        footer.getRowIndexStride(), conf, file);
  }
