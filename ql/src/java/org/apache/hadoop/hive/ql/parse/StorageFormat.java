...
public class StorageFormat {
  ...
  protected void processStorageFormat(String name) throws SemanticException {
    ...
    if (serde == null) {
      ...
      // AvroFile also supports a configurable SerDe
      else if ((name.equalsIgnoreCase(IOConstants.AVROFILE)) || (name.equalsIgnoreCase(IOConstants.AVRO)))  {
        String avroserdeClass = HiveConf.getVar(conf, HiveConf.ConfVars.HIVEDEFAULTAVROFILESERDE);
        if (!StringUtils.isBlank(avroserdeClass)) {
          serde = ensureClassExists(avroserdeClass);
        }
      } else {
  ...
  protected void fillDefaultStorageFormat(boolean isExternal) throws SemanticException {
	...
	defaultFormat = defaultManagedFormat;
      ...
      if (StringUtils.isBlank(defaultFormat)) {
        ...
        if ((defaultFormat.equalsIgnoreCase(IOConstants.AVROFILE)) || 
                 (defaultFormat.equalsIgnoreCase(IOConstants.AVRO))) {
          String defaultavroSerde = HiveConf.getVar(conf, HiveConf.ConfVars.HIVEDEFAULTAVROFILESERDE);
          if (!StringUtils.isBlank(defaultavroSerde)) {
            // use the default avro serde
            serde = defaultavroSerde;
          }
        }
       }
    }
