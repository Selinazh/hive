...
public class FetchOperator implements Serializable {
  ...
  static final int LOAD_PARTITION_BATCH_SIZE = 2;
  ...
  private transient List<Path> listP = new ArrayList<Path>();
  private transient List<PartitionDesc> partP = new ArrayList<PartitionDesc>();
  ...
  private boolean getNextPath() throws Exception {
    // if partition objects not loaded yet, load them in batch.
    loadPartitions();
  ...
  private void loadPartitions() throws Exception {
    if (!iterPath.hasNext() && work.getPartNames() != null && !work.getPartNames().isEmpty()) {
      List<String> batch = work.getPartNames().
          subList(0, Math.min(LOAD_PARTITION_BATCH_SIZE, work.getPartNames().size()));
      List<Partition> partitions = new ArrayList<Partition>();
      if (!batch.isEmpty()) {
        // we can assume this is a simple fetch task called from local job, safe to make metastore call
        Table table = Hive.get().getTable(work.getTblDesc().getTableName());
        partitions.addAll(Hive.get().getPartitionsByNames(table, batch));
      }
      listP = new ArrayList<Path>();
      partP = new ArrayList<PartitionDesc>();
      
      for (Partition partition : partitions) {
        listP.add(partition.getDataLocation());
        partP.add(Utilities.getPartitionDesc(partition));
      }
      iterPath = listP.iterator();
      iterPartDesc = partP.iterator();
      batch.clear();
    }
  }
  ...
  private StructObjectInspector setupOutputObjectInspector() throws HiveException {
    ...
    try {
      ...
      // for lazy loaded partition objects, always use converted outputOI
      if (work.getPartNames() != null){
