...
public class Driver implements CommandProcessor {
  ...
  public static void doAuthorization(BaseSemanticAnalyzer sem, String command)
    ...
    if (inputs != null && inputs.size() > 0) {
      Map<String, List<String>> tab2Cols = new HashMap<String, List<String>>();
      Map<String, List<String>> part2Cols = new HashMap<String, List<String>>();
      ...
      for (ReadEntity read : inputs) {
        ...
        if (read.getPartition() != null) {
          ...
          if (Boolean.TRUE.equals(tableUsePartLevelAuth.get(tbl.getTableName()))) {
            String key = tbl.getDbName() + "." + tbl.getTableName() + "." + partition.getName();
            List<String> cols = part2Cols.get(key);
        ...
        if (tbl != null && !tableAuthChecked.contains(tbl.getTableName()) &&
          ...
          String key = tbl.getDbName() + "." + tbl.getTableName();
          List<String> cols = tab2Cols.get(key);
          if (cols != null && cols.size() > 0) {
  ...
  private static void getTablePartitionUsedColumns(HiveOperation op, BaseSemanticAnalyzer sem,
      Map<String, List<String>> tab2Cols, Map<String, List<String>> part2Cols,
        ...
        if (topOp instanceof TableScanOperator) {
          ...
          if (tbl.isPartitioned() &&
            ...
            for (Partition part : parts) {
              String key = part.getTable().getDbName() + "." + part.getTable().getTableName() + "." + part.getName();
              List<String> existingCols = part2Cols.get(part);
              if (existingCols == null) {
                existingCols = new ArrayList<String>();
              }
              existingCols.addAll(cols);
              part2Cols.put(key, existingCols);
            }
          } else {
            String key = tbl.getDbName() + "." + tbl.getTableName();
            List<String> existingCols = tab2Cols.get(key);
            if (existingCols == null) {
              existingCols = new ArrayList<String>();
            }
            existingCols.addAll(cols);
            tab2Cols.put(key, existingCols);
          }
        }
      }
    }
