...
@after { popMsg(state); }
    : (add=KW_ADD KW_COLUMNS ifNotExists? | replace=KW_REPLACE KW_COLUMNS) LPAREN columnNameTypeList RPAREN restrictOrCascade?
    -> {$add != null}? ^(TOK_ALTERTABLE_ADDCOLS ifNotExists? columnNameTypeList restrictOrCascade?)
