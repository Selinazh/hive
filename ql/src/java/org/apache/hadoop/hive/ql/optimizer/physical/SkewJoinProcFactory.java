...
public final class SkewJoinProcFactory {
  ...
  public static class SkewJoinJoinProcessor implements NodeProcessor {
    public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx ctx,
      ...
      if (currentTsk.getWork() instanceof MapredWork) {
        JoinOperator op = (JoinOperator) nd;
        if (op.getConf().isFixedAsSorted()) {
          return null;
        }
        GenMRSkewJoinProcessor.processSkewJoin(op, currentTsk, parseContext);
      } else if (currentTsk.getWork() instanceof TezWork){
        CommonMergeJoinOperator op = (CommonMergeJoinOperator) nd;
        GenTezSkewJoinProcessor.processSkewJoin(op, currentTsk, parseContext);
      }
      return null;
    }
  }
