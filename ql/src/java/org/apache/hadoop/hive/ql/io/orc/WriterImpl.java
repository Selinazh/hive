...
public class WriterImpl implements Writer, MemoryManager.Callback {
  ...
  WriterImpl(FileSystem fs,
    ...
    LOG.info("ORC writer created for path: " + path + " with stripeSize: " + defaultStripeSize + " blockSize: " +
        blockSize + " compression: " + compress + " estimatedBufferSize: " + this.bufferSize);
  }
  ...
  private abstract static class TreeWriter {
    ...
    protected boolean disableDictionary = false;
    protected boolean disableDictionaryEncoding = false;
    ...
    TreeWriter(int columnId, ObjectInspector inspector,
      ...
      this.disableDictionary = streamFactory.getConfiguration().getBoolean(
          HiveConf.ConfVars.HIVE_ORC_DISABLE_DICTIONARY_COMPLEX_TYPES.varname,
          HiveConf.ConfVars.HIVE_ORC_DISABLE_DICTIONARY_COMPLEX_TYPES.defaultBoolVal);
    }
    ...
    void setDisableDictionaryEncoding(boolean disableDictionary) {
      // Disable dictionary encoding only if specified by configs.
      if (this.disableDictionary) {
        LOG.info("Disabling dictionary for column id: " + this.id);
        disableDictionaryEncoding = disableDictionary;
      }
    }
  }
    ...
    private boolean checkDictionaryEncoding() {
      if (!doneDictionaryCheck) {
        if (disableDictionaryEncoding) {
          // Enable dictionary encoding until find a not null value.
  ...
  private static class StructTreeWriter extends TreeWriter {
    ...
    StructTreeWriter(int columnId,
      ...
      for(int i=0; i < childrenWriters.length; ++i) {
        ...
        if (columnId != 0) {
          childrenWriters[i].setDisableDictionaryEncoding(true);
        }
      }
      recordPosition(rowIndexPosition);
    }
    ...
    ListTreeWriter(int columnId,
      ...
      childrenWriters[0] =
        createTreeWriter(listObjectInspector.getListElementObjectInspector(),
          writer, true);
      childrenWriters[0].setDisableDictionaryEncoding(true);
  ...
  private static class MapTreeWriter extends TreeWriter {
    ...
    MapTreeWriter(int columnId,
      ...
      childrenWriters[1].setDisableDictionaryEncoding(true);
      lengths = createIntegerWriter(writer.createStream(columnId,
          OrcProto.Stream.Kind.LENGTH), false, isDirectV2, writer);
      recordPosition(rowIndexPosition);
    }
