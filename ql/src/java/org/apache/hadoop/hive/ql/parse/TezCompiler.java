...
import org.apache.hadoop.hive.ql.optimizer.MetadataOnlyOptimizer;
...
public class TezCompiler extends TaskCompiler {
  ...
  protected void optimizeTaskPlan(List<Task<? extends Serializable>> rootTasks, ParseContext pCtx,
    ...
    if (conf.getBoolVar(HiveConf.ConfVars.HIVESKEWJOIN)) {
      physicalCtx = new SkewJoinResolver().resolve(physicalCtx);
    } else {
      LOG.debug("Skipping skew join query optimization");
    }
