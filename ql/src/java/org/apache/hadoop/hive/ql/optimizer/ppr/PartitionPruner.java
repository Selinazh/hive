...
public class PartitionPruner implements Transform {
  ...
  public static PrunedPartitionList prune(TableScanOperator ts, ParseContext parseCtx,
    ...
    ExprNodeDesc prunerExpr = parseCtx.getOpToPartPruner().get(ts);
    ts.getConf().setPartPrunerExpr(compactExpr(prunerExpr));
    return ts.getIsMetadataOnly()?
              getPartitionNamesFromServer(table, prunerExpr, parseCtx.getConf(), parseCtx.getPrunedPartitions())
            : prune(table, prunerExpr, parseCtx.getConf(), alias, parseCtx.getPrunedPartitions());
  }
  ...
  public static PrunedPartitionList prune(Table tab, ExprNodeDesc prunerExpr,
    ...
    LOG.debug("Getting partitions for Table:" + tab.getCompleteName() + " using prunerExpr == " + (prunerExpr == null? "null" : prunerExpr.getExprString()));
    if (!tab.isPartitioned()) {
      // If the table is not partitioned, return empty list.
      LOG.debug("Default handling for unpartitioned table. \"Return Everything.\"");
    ...
    if (prunerExpr == null) {
      ...
      LOG.debug("Null prune-expression. Getting all partitions names.");
      List<String> partNames = getAllPartitionNamesImpl(tab);
      return new PrunedPartitionList(tab, null, Sets.newLinkedHashSet(partNames), false, PrunedPartitionList.loadTypes.LAZY_LOAD);
    }
    ...
    if (compactExpr == null || isBooleanExpr(compactExpr)) {
      ...
      LOG.debug("Filter " + oldFilter + " was null after compacting. Getting all partitions.");
      List<String> partNames = getAllPartitionNamesImpl(tab);
      return new PrunedPartitionList(tab, null, Sets.newLinkedHashSet(partNames), false, PrunedPartitionList.loadTypes.LAZY_LOAD);
    }
  ...
  private static PrunedPartitionList getPartitionsFromServer(Table tab,
    ...
    try {
      ...
      if (!doEvalClientSide) {
        ...
        try {
          hasUnknownPartitions = Hive.get().getPartitionsByExpr(tab, compactExpr, conf, (short) -1, partitions);
  ...
  public static Set<Partition> getAllPartitions(Table tab) throws HiveException {
    PerfLogger perfLogger = PerfLogger.getPerfLogger();
    perfLogger.PerfLogBegin(CLASS_NAME, PerfLogger.PARTITION_RETRIEVING);
  ...
  private static PrunedPartitionList getPartitionNamesFromServer(Table table,
      ExprNodeDesc prunerExpr, HiveConf conf, Map<String, PrunedPartitionList> prunedPartitionsMap) throws SemanticException {
    try {
      LOG.debug("Fetching partition names for Table: " + table.getCompleteName() + " using prunerExpr == " + (prunerExpr == null? "null" : prunerExpr.getExprString()));
      String key = table.getDbName() + "." + table.getTableName() + ";";
      if (!table.isPartitioned()) {
        LOG.debug("Default handling for unpartitioned table.");
        return getAllPartsFromCacheOrServer(table, key, false, prunedPartitionsMap);
      }

      if (prunerExpr == null) {
        LOG.debug("Null prune-expression. Returning all partition names.");
        return new PrunedPartitionList(table, null, Sets.newLinkedHashSet(getAllPartitionNamesImpl(table)), false, PrunedPartitionList.loadTypes.SKIP_LOAD);
      }

      // Remove virtual columns. See javadoc for details.
      List<String> partitionColumnNames = extractPartColNames(table);
      Set<String> partColsUsedInFilter = new LinkedHashSet<String>();
      prunerExpr = removeNonPartCols(prunerExpr, partitionColumnNames, partColsUsedInFilter);
      ...
      String oldFilter = prunerExpr.getExprString();
      if (compactExpr == null) {
        LOG.debug("Filter " + oldFilter + " was null after compacting. Returning all partition-names.");
        return new PrunedPartitionList(table, new ArrayList<String>(partColsUsedInFilter), Sets.newLinkedHashSet(getAllPartitionNamesImpl(table)), false, PrunedPartitionList.loadTypes.SKIP_LOAD);
      }
      else {
        LOG.debug("After compacting, filter == " + compactExpr.getExprString() + ". Finding pruned list of partition-names.");
        List<String> partNames = getAllPartitionNamesImpl(table);
        List<PrimitiveTypeInfo> partColTypeInfos = extractPartColTypes(table);
        boolean hasUnknowns = prunePartitionNames(partitionColumnNames,
                                                  partColTypeInfos,
                                                  compactExpr,
                                                  conf.getVar(HiveConf.ConfVars.DEFAULTPARTITIONNAME),
                                                  partNames);
        return new PrunedPartitionList(table, new ArrayList<String>(partColsUsedInFilter), Sets.newLinkedHashSet(partNames), hasUnknowns, PrunedPartitionList.loadTypes.SKIP_LOAD);
      }
    } catch(SemanticException e) {
      throw e;
    } catch(Exception e) {
  ...
  private static List<String> getAllPartitionNamesImpl(Table table) throws SemanticException {
    try {
      return getPartitionNamesImpl(table, (short)-1);
    }
    catch (HiveException e) {
      throw new SemanticException(e);
    }
  ...
  private static List<String> getPartitionNamesImpl(Table table, short maxPartitions) throws HiveException {
    return Hive.get().getPartitionNames(table.getDbName(), table.getTableName(), maxPartitions);
  }
  ...
  static private boolean pruneBySequentialScan(Table tab, List<Partition> partitions,
    ...
    int maxPartitionsPermitted = conf.getInt(HiveConf.ConfVars.HIVELIMITTABLESCANPARTITION.varname, -1);
    if (maxPartitionsPermitted != -1) {
      LOG.debug("Max partitions permitted == " + maxPartitionsPermitted + ". Enforcing limit...");
      if (partNames.size() > maxPartitionsPermitted) {
        throw new SemanticException("Too many partitions (>=" + partNames.size() + ") selected from "
            + tab.getCompleteName() + ". Please specify a more restrictive partition-selection clause.");
      }
    }
  ...
  public static List<String> extractPartColNames(Table tab) {
