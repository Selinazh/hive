...
public class TezTask extends Task<TezWork> {
  ...
  public int execute(DriverContext driverContext) {
    ...
    try {
      ...
      // Unset MR/Tez credential paths similar to MAPREDUCE-3727
      TokenCache.cleanUpTokenReferral(jobConf);
      jobConf.unset(TezConfiguration.TEZ_CREDENTIALS_PATH);
      ...
      driverContext.checkShutdown(); // Avoiding lost work: Checking for shutdown before submitting job.
      ...
      console.printError("Failed to execute tez graph. Reason: " +  e.getMessage());
  ...
  DAGClient submit(JobConf conf, DAG dag, Path scratchDir,
    ...
    try {
      try {
        ...
        LOG.info("Attempting to submit DAG to Tez AM: applicationId="
            + sessionState.getSession().getAppMasterApplicationId()
            + " status=" + sessionState.getSession().getAppMasterStatus());
