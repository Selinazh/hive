...
public abstract class InStream extends InputStream {
  ...
  private static class CompressedStream extends InStream {
    ...
    private void readHeader() throws IOException {
      ...
      if (compressed.remaining() > OutStream.HEADER_SIZE) {
        ...
        if (chunkLength > bufferSize) {
          // alter table concatenate had an issue setting max buffer size incorrectly. HIVE-13361 had fixed it.
          // We need to read file generated before the fix.
          LOG.warn("Buffer size too small. size = " + bufferSize + " needed = " + chunkLength);
        }
        ...
        if (isOriginal) {
          ...
          if (isUncompressedOriginal) {
            createBufferAndDecompress(codec, slice);
    ...
    private void createBufferAndDecompress(CompressionCodec codec, ByteBuffer slice) throws IOException {
      if (slice.isDirect()) {
        uncompressed = allocateBuffer(bufferSize, slice.isDirect());
        codec.decompress(slice, uncompressed);
      } else {
        ChannelBuffer buf = ChannelBuffers.dynamicBuffer();
        codec.decompress(slice, buf, bufferSize);
        uncompressed = buf.toByteBuffer();
      }
    }
