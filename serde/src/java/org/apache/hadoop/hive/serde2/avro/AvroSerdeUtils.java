...
public class AvroSerdeUtils {
  ...
  public static final String TABLE_CREATION_EXCEPTION_MESSAGE = "Neither column were specified nor "
      + AvroTableProperties.SCHEMA_LITERAL.getPropName() + " nor "
      + AvroTableProperties.SCHEMA_URL.getPropName() + " supplied in TBLPROPERTIES(during creation) hence can't determine table schema ";
  ...
  public static Schema determineSchemaOrThrowException(Configuration conf, Properties properties)
    ...
    if(schemaString != null && !schemaString.equals(SCHEMA_NONE)) {
      LOG.debug("AvroSerdeUtils::determineSchemaOrThrowException(): Resolving from "
    ...
    try {
      LOG.debug("AvroSerdeUtils::determineSchemaOrThrowException(): Attempting to resolve schema from "
               + AvroTableProperties.SCHEMA_URL.getPropName() + " (" + schemaString + ")." );
