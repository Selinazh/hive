...
public void testParseUrlHttpMode() throws SQLException, JdbcUriParseException,
  ...
  public void testGetQueryLog() throws Exception {
    ...
    List<String> logs = stmt.getQueryLog(false, 20000);
    stmt.close();
    ...
    incrementalLogs.clear();
  }

  /**
   * Returns true if all of the strings in expectedLogs are found in logs.
   * The order of the expectedLogs is maintained, such that the first expected string
   * must appear before the second expected string and so on.
   * @param logs the server logs messages.
   * @param expectedLogs the expected strings.
   * @return true if all the expected string are found in the log messages.
   */
  private void verifyFetchedLog(List<String> logs, String[] expectedLogs) {
    ...
    for (String expected : expectedLogs) {
      ...
      assertTrue("Unable to find '" + expected + "' in " + i + " of " + logs.size() + " query logs.", found);
    }
  }
