...
public class TestHiveSessionImpl {
  ...
  public void testLeakOperationHandle() throws HiveSQLException {
    ...
    HandleIdentifier handleId = Mockito.mock(HandleIdentifier.class);
    Mockito.when(operation.getHandle()).thenReturn(opHandle);
    Mockito.when(opHandle.getHandleIdentifier()).thenReturn(handleId);
    Mockito.when(handleId.toString()).thenReturn("MOCK-ID");
