...
public class HadoopThriftAuthBridge {
  ...
  public static class Server {
    ...
    public TTransportFactory createTransportFactory(Map<String, String> saslProps, Configuration conf)
        throws TTransportException {
      ...
      transFactory.addServerDefinition(
          ...
          saslProps,
          new SaslGssCallbackHandler(conf));
    ...
    static class SaslGssCallbackHandler extends SaslRpcServer.SaslGssCallbackHandler implements CallbackHandler {

      private final Set<String> whitelistGroups = new HashSet<String>();
      public static final String WHITELIST_GROUP_KEYNAME =
              "hive.server2.thrift.access.whitelist.groups";
      ...
      public SaslGssCallbackHandler(Configuration conf) {
        ...
        if (conf != null) {
          ...
          if(groups != null && !groups.isEmpty()) {
            ...
            LOG.info("Whitelist groups for hive server 2 are " + whitelistGroups);
          } else {
            LOG.info("No whilelist group " + WHITELIST_GROUP_KEYNAME + " configured for HiveServer2");
          }
        }
      }
        ...
        if (ac != null) {
          if (ac.isAuthorized()) {
            ...
            if(whitelistGroups.size() > 0) {
              UserGroupInformation remoteUser = UserGroupInformation.createRemoteUser(authid);
              for (String group : remoteUser.getGroupNames()) {
                if (group != null && whitelistGroups.contains(group)) {
                  authorized = true;
                  LOG.debug("User is part of whitelist group Will grant access");
                  break;
                }
              }
              if (!authorized) {
                ac.setAuthorized(false);
                LOG.info("User not part of whitelist group: " + whitelistGroups + ". Will deny access");
              }
            }
          }
        }
    ...
    protected class TUGIAssumingProcessor implements TProcessor {
      ...
      // UserGroupInformation equals method is broken and does not match instances
      // which are logically the same. Must maintain a single instance to ensure
      // we can clean up FileSystem.Cache.
      ...
      public boolean process(final TProtocol inProt, final TProtocol outProt) throws TException {
        ...
        if (trans instanceof TSaslServerTransport) {
          ...
          if (exception == null) {
            ...
            try {
              if (useProxy) {
                remoteUser.set(getClientUgi(endUser).getShortUserName());
                LOG.debug("Set remoteUser :" + remoteUser.get());
                rc = getClientUgi(endUser).doAs(new PrivilegedExceptionAction<Boolean>() {
                  @Override
                  public Boolean run() {
                    try {
                ...
                rc = wrapped.process(inProt, outProt);
              }
              ...
              runtimeException = new RuntimeException(e); // Unexpected.
            } catch (IOException e) {
              runtimeException = new RuntimeException(e); // Unexpected.
            } catch (TException e) {
              exception =e;
              ...
              runtimeException = new RuntimeException(e); // When all else fails.
            }
          }
        } else {
        ...
        if (! rc) {
          // Connection was closed or an exception occurred which will stop the WorkerProcess
          // Clean FileSystem.Cache and reset the clientUgi.
          if (this.clientUgi.get() != null) {
            LOG.debug("Closing FileSystem for UGI: " + this.clientUgi.get());
            try {
              FileSystem.closeAllForUGI(this.clientUgi.get());
            } catch (IOException e) {
              LOG.error("Could not clean up file-system handles for UGI: " + this.clientUgi.get(), e);
            } finally {
      ...
      private synchronized UserGroupInformation getClientUgi(String endUser) throws IOException {
        if (this.clientUgi.get() == null) {
          this.clientUgi.set(UserGroupInformation.createProxyUser(
