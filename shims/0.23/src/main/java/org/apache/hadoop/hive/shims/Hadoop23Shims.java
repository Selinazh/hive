...
public class Hadoop23Shims extends HadoopShimsSecure {
  ...
  public void setFullFileStatus(Configuration conf, HdfsFileStatus sourceStatus,
    ...
    try {
      ...
      if (hasExtendedAcls((Hadoop23FileStatus) sourceStatus)) {
        List<AclEntry> aclEntries = getDefaultAclEntries((Hadoop23FileStatus) sourceStatus);
  ...
  /**
   * Generates a list of <code>AclEntry</code>s as determined from <code>src</code>.
   * First iterate through all of the <code>AclEntry</code>s and extract the DEFAULT rules, from which
   * we will create an ACCESS rule as well. If DEFAULT rules were not found for user, group, other, or
   * mask, then inherit them from the basic permissions of <code>src</code>. Because the group permission
   * is now the mask, we need to pull the group permission from list of extended ACLs.
   *
   * @param src the <code>FileStatus</code> from which to inherit permissions.
   * @return a list of <code>AclEntry</code>s to inherit from <code>src</code>.
   */
  private List<AclEntry> getDefaultAclEntries(Hadoop23FileStatus src) {
    ArrayList<AclEntry> defaults = new ArrayList<AclEntry>();

    boolean[] hasDefaults = { false, false, false, false };

    for (AclEntry e : src.getAclStatus().getEntries()) {
    ...
    if (! hasDefaults[0]) {
      AclEntry acl = new AclEntry.Builder().setScope(AclEntryScope.ACCESS)
        .setType(AclEntryType.USER).setPermission(src.getFileStatus().getPermission().getUserAction()).build();
      defaults.add(acl);
    ...
    if (! hasDefaults[2]) {
      AclEntry acl = new AclEntry.Builder().setScope(AclEntryScope.ACCESS)
        .setType(AclEntryType.OTHER).setPermission(src.getFileStatus().getPermission().getOtherAction()).build();
      defaults.add(acl);
    ...
    if (! hasDefaults[3]) {
      AclEntry acl = new AclEntry.Builder().setScope(AclEntryScope.ACCESS)
        .setType(AclEntryType.MASK).setPermission(src.getFileStatus().getPermission().getGroupAction()).build();
      defaults.add(acl);
  ...
  /**
   * Returns <code>true</code> if extended ACL rules exist, otherwise <code>false</code>.
   * @param status the <code>FileStatus</code> containing access permissions.
   * @return <code>true</code> if extended ACL rules exist, otherwise <code>false</code>.
   */
  private boolean hasExtendedAcls(Hadoop23FileStatus status) {
    if (status.getAclStatus() != null) {
      return status.getAclStatus().getEntries().size() > 0;
    }
