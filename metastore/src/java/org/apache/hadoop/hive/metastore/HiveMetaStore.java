...
import org.apache.hadoop.hive.thrift.TServerSocketKeepAlive;
...
public class HiveMetaStore extends ThriftHiveMetastore {
  ...
  public static class HMSHandler extends FacebookBase implements IHMSHandler, ThreadLocalRawStore {
    ...
    public String startDatabaseFunction(String function, String db) {
      return startFunction(function, " db=" + db);
    }

    public String startTypeFunction(String function, String type) {
      return startFunction(function, " type=" + type);
    }
    ...
    private String startTableFunction(String function, String db, String tbl, String filter) {
      return startFunction(function, " db=" + db + " tbl=" + tbl + " filter=[" + filter + "]");
    }

    private String startTableFunction(String function, String db, String tbl, Partition partition) {
      return startFunction(function, " db=" + db + " tbl=" + tbl + " partition=" +
          "[" + join(partition.getValues(), ",") + "]");
    }
    ...
    private String startPartitionFunction(String function, String db, String tbl,
      ...
      return startFunction(function, " db=" + db + " tbl=" + tbl
          + " partition=[" + join(partVals, ",") + "]");
    }

    public String startPartitionListFunction(String function, String db, String tbl,
        List<Partition> partitions) {
      return startFunction(function, " db=" + db + " tbl=" + tbl + " size=" + partitions.size()
          + " partitions={" + joinParts(partitions, ",") + "}");
    }

    private String joinParts(List<Partition> partitions, String s) {
      StringBuilder builder = new StringBuilder(1024);
      for (Partition part : partitions) {
        builder.append("[").append(join(part.getValues(), ",")).append("]").append(s);
      }
    ...
    public String startColumnFunction(String function, String db, String tbl,
      String column) {
      return startFunction(function, " db=" + db + " tbl=" + tbl + " col=" + column);
    }

    public String startPartitionColumnFunction(String function, String db, String tbl,
      String partName, String column) {
      return startFunction(function, " db=" + db + " tbl=" + tbl + " partition=" + partName
          + " col=" + column);
    }
    ...
    public void create_database(final Database db)
        throws AlreadyExistsException, InvalidObjectException, MetaException {
      startFunction("create_database", " db=" + db.getName() + " value=" + db.toString());
      boolean success = false;
      Exception ex = null;
      try {
        try {
    ...
    public Database get_database(final String name) throws NoSuchObjectException, MetaException {
      startDatabaseFunction("get_database", name);
      Database db = null;
      Exception ex = null;
      try {
    ...
    public void alter_database(final String dbName, final Database newDB)
      ...
      startDatabaseFunction("alter_database", dbName);
      boolean success = false;
      Exception ex = null;
      ...
      try {
        Database oldDB = get_database(dbName);
        if (oldDB == null) {
    ...
    private void drop_database_core(RawStore ms,
      ...
      try {
          ...
          if (tables != null && !tables.isEmpty()) {
            for (Table table : tables) {
              ...
              partitionPaths = dropPartitionsAndGetLocations(ms, name, table.getTableName(),
                  tablePath, table.getPartitionKeys(), false, deleteData && !isExternal(table));
        ...
        if (!success) {
          ...
          try {
            //before deleting the parent path please check if there is
            //data in the underlying directory
            wh.deleteDir(new Path(db.getLocationUri()),
                         true,  // Recursive delete.
                         false, // skip-trash on deletion.
                         true   // Delete database-directory only if empty.
                        );
          } catch (Exception e) {
    ...
    public void drop_database(final String dbName, final boolean deleteData, final boolean cascade)
      ...
      startDatabaseFunction("drop_database", dbName);
    ...
    public boolean create_type(final Type type) throws AlreadyExistsException,
      ...
      startTypeFunction("create_type", type.toString());
      boolean success = false;
      Exception ex = null;
      try {
    ...
    public Type get_type(final String name) throws MetaException, NoSuchObjectException {
      startTypeFunction("get_type", name);
    ...
    public boolean drop_type(final String name) throws MetaException, NoSuchObjectException {
      startTypeFunction("drop_type", name);
    ...
    public Map<String, Type> get_type_all(String name) throws MetaException {
      ...
      startTypeFunction("get_type_all", name);
    ...
    public void create_table_with_environment_context(final Table tbl,
        final EnvironmentContext envContext)
        throws AlreadyExistsException, MetaException, InvalidObjectException {
      startFunction("create_table", " db=" + tbl.getDbName() + " tbl=" + tbl.getTableName() + " value=" + tbl.toString());
      boolean success = false;
      Exception ex = null;
      try {
    ...
    private boolean drop_table_core(final RawStore ms, final String dbname, final String name,
      ...
      // try drop all partitions in once using directSQL
      try {
        return dropTableUsingdirectsql(ms, dbname, name, deleteData, envContext, indexName);
      }catch (Exception e) {
        if (e instanceof MetaException) {
          LOG.error("drop table using directSQL failed for table:" + name, e);
          throw (MetaException) e;
        }
        LOG.info("fall back to ORM, can't drop all partitions in once using directSQL: " + e.getMessage());
      }
      ...
      try {
        ...
        if (tbl.getPartitionKeysSize() > 0) {
          ms.canDropAllPartitions(dbname, name, false);
        }
        if (!isIndexTable) {
          try {
        ...
        // drop all partitions for partitioned table
        if (tbl.getPartitionKeysSize() > 0) {
            partPaths = dropPartitionsAndGetLocations(ms, dbname, name, tblPath,
                tbl.getPartitionKeys(), false, deleteData && !isExternal);
        }
    ...
    private boolean dropTableUsingdirectsql(final RawStore ms, final String dbname, final String name,
      ...
      try {
        ...
        // For partitioned table, check if the partitions can be dropped
        if (tbl.getPartitionKeysSize() > 0) {
          ms.canDropAllPartitions(dbname, name, true);
        }
        if (!isIndexTable) {
          try {
            List<Index> indexes = ms.getIndexes(dbname, name, Short.MAX_VALUE);
        ...
        // drop all partitions for partitioned table
        if (tbl.getPartitionKeysSize() > 0) {
          // try drop all partitions in once first
          partPaths = dropPartitionsAndGetLocations(ms, dbname, name, tblPath,
              tbl.getPartitionKeys(), true, deleteData && !isExternal);
        }
        // drop table object
        if (!ms.dropTable(dbname, name)) {
    ...
    */
    ...
    private List<Path> dropPartitionsAndGetLocations(RawStore ms, String dbName,
      String tableName, Path tablePath, List<FieldSchema> partitionKeys, boolean allowsql, boolean checkLocation)
      throws MetaException, IOException, NoSuchObjectException, InvalidObjectException,
      InvalidInputException {
      List<Path> partPaths = new ArrayList<Path>();
      partPaths =  ms.dropAllPartitionsNoTxn(dbName, tableName, checkLocation, allowsql, tablePath);
      // check if each location is writable
      ...
      if (partPaths != null) {
        for (Path path: partPaths) {
          Path partPath = wh.getDnsPath(path);
          if (partPath != null && !wh.isWritable(partPath.getParent())) {
            throw new MetaException("Table metadata not deleted since the partition path " + partPath
                + " has parent location " + partPath.getParent() + " which is not writable"
                + " by " + hiveConf.getUser());
          }
          // check use dns path because object store only check file path
          if (tableDnsPath != null && isSubdirectory(tableDnsPath, partPath)) {
            partPaths.remove(path);
          }
        }
      }
      return partPaths;
    ...
    /**
     * Searches tables in hive metastore and obtains table objects
     *
     * @param SearchRequest - The search request containing the keywords
     *
     * @return SearchResponse - A list of table objects in metastore
     *                          which match the keywords. No duplicates.
     * @throws MetaException
     */
    @Override
    public SearchResponse search_tables(SearchRequest request) throws MetaException {
      ...
      try {
        startFunction("search_tables");

        // Are the keywords OK?
        if (request == null || request.getQuery() == null || request.getQuery().isEmpty()) {
          throw new IllegalArgumentException("Search request or query empty, cannot search");
        }

        List<Table> results = getMS().listTablesByQuery(request.getQuery().split(" "), request.getMaxResults());
        searchResponse.setTables(results);
        success = true;

      } catch (Exception e) {
        ex = e;
        LOG.error("Error searching for tables", e);
        throw newMetaException(e);

      } finally {
        endFunction("search_tables", success, ex);
      }
    ...
    private List<Partition> add_partitions_core(
      ...
      TryAddPartitionsCoreState state = try_add_partitions_core(
          ms,
          dbName,
          tblName,
          parts,
      ...
      try {
        add_partitions_core_epilogue(state); // Throws, if cleanup throws.

        // Epilogue succeeded. If there's a pending exception from try_add_partitions_core(), throw it.
        if (state.exception != null) {
          throw state.exception;
        }

        // No exceptions. Return the result.
        return state.result;
      }
      finally {
        endFunction("add_partitions_core", state.success, state.exception, tblName);
      }
    }
    ...
    public int add_partitions(final List<Partition> parts) throws MetaException,
      ...
      // The assumption is all partitions belong to the same table.
      startPartitionListFunction("add_partitions", parts.get(0).getDbName(),
          parts.get(0).getTableName(), parts);
    ...
    public Partition add_partition_with_environment_context(
      ...
      startTableFunction("add_partition",
          part.getDbName(), part.getTableName(), part);
      Partition ret = null;
      Exception ex = null;
      try {
    ...
    public DropPartitionsResult drop_partitions_req(
      ...
      logInfo("drop_partitions_req");

      TryDropPartitionsState state = try_drop_partitions_req(getMS(), request);
      try {
        drop_partitions_req_epilogue(state); // Throws, if cleanup throws.

        // Epilogue succeeded. If there's a pending exception from try_drop_partitions_req(), throw it.
        if (state.exception != null) {
          throw state.exception;
        }

        // No exceptions. Return the result.
        return state.dropPartitionsResult;
      }
      finally {
        endFunction("drop_partitions", state.success, state.exception, request.getTblName());
      }
    }
    ...
    // drop_and_add_partitions_req
    @Override
    public void drop_and_add_partitions_req(
                                    DropPartitionsRequest dropPartitionsRequest,
                                    AddPartitionsRequest addPartitionsRequest)
                                    throws TException {
      logInfo("drop_and_add_partitions_req");
      RawStore ms = getMS();

      TException tException = null;

      ms.openTransaction(); // Overarching transaction.

      LOG.info("Attempting to drop partitions.");
      TryDropPartitionsState dropPartitionsState = try_drop_partitions_req(ms, dropPartitionsRequest);

      if (!dropPartitionsState.success) {
        LOG.error("Could not drop partitions in drop_and_add_partitions_req(). Exception :",
            dropPartitionsState.exception);
        drop_partitions_req_epilogue(dropPartitionsState);
        if (dropPartitionsState.exception != null) {
          throw dropPartitionsState.exception;
        }
        return;
      }

      LOG.info("Partitions were dropped correctly. Attempting to add partitions.");

      TryAddPartitionsCoreState addPartitionsState = try_add_partitions_core(
          ms,
          addPartitionsRequest.getDbName(),
          addPartitionsRequest.getTblName(),
          addPartitionsRequest.getParts(),
          addPartitionsRequest.isIfNotExists());

      tException = addPartitionsState.exception;

      // At this point, try_drop_partitions_req() has succeeded already.
      // If try_add_partitions_core() has succeeded as well, commit now.
      // NOTE: Commit on success, before cleanup, as per convention in add_partitions(), etc.
      // Also note: In case try_add_partitions_core() or try_drop_partitions_req() fail,
      // there is no need to roll the transaction back here.
      // The individual try_*() functions would have rolled back, so any rollbackTransaction() here would be a NOOP.

      boolean dropAndAddSuccess = addPartitionsState.success && ms.commitTransaction(); // && dropPartitionsState.success!

      // Attempt epilogues.
      // NOTE: First add_partitions_core_epilogue(), then drop_partitions_req_epilogue().
      // Why? drop_partitions_req_epilogue() deletes data on success. Attempt only if add_partitions*() goes through.

      // 1. Add partitions epilogue.
      try {
        // If drop/add/commit failed, the epilogue must assume the failure.
        addPartitionsState.success(dropAndAddSuccess);
        add_partitions_core_epilogue(addPartitionsState);
      }
      catch (TException exception) {
        LOG.error("add_partitions_core_epilogue failed!", exception);
        dropAndAddSuccess = false; // Add-partitions cleanup failed.
        tException = exception;
      }

      // 2. Drop partitions epilogue.
      try {
        // If try_drop/try_add/commit/drop_epilogue failed, drop_partitions_req_epilogue must assume failure.
        assert dropPartitionsState.success : "At this point, try_drop_partitions_req should've already succeeded.";
        dropPartitionsState.success(dropAndAddSuccess);
        drop_partitions_req_epilogue(dropPartitionsState);
      }
      catch (TException exception) {
        LOG.error("drop_partitions_req_epilogue failed!", exception);
        tException = exception; // Drop-partitions cleanup failed.
      }

      if (tException != null) {
        throw tException; // Throw last failure.
      }

      // No need finally { ms.rollback(); } on error, since the try_*() methods would have already rolled back on error.
    }

    private static class TryDropPartitionsState {
      // State. Might require cleanup.
      EnvironmentContext envContext = null;
      Table table = null;
      List<Partition> parts = null;
      List<PathAndPartValSize> dirsToDelete = null;
      List<Path> archToDelete = null;
      boolean deleteData = false, mustPurge = false;

      // Results.
      DropPartitionsResult dropPartitionsResult = null;
      TException exception = null;
      boolean success = false;

      TryDropPartitionsState envContext(EnvironmentContext envContext)
      { this.envContext = envContext; return this; }

      TryDropPartitionsState table(Table table)
      { this.table = table; return this; }

      TryDropPartitionsState parts(List<Partition> parts)
      { this.parts = parts; return this; }

      TryDropPartitionsState dirsToDelete(List<PathAndPartValSize> dirsToDelete)
      { this.dirsToDelete = dirsToDelete; return this; }

      TryDropPartitionsState archToDelete(List<Path> archToDelete)
      { this.archToDelete = archToDelete; return this; }

      TryDropPartitionsState deleteData(boolean deleteData)
      { this.deleteData = deleteData; return this; }

      TryDropPartitionsState mustPurge(boolean mustPurge)
      { this.mustPurge = mustPurge; return this; }

      TryDropPartitionsState dropPartitionsResult(DropPartitionsResult dropPartitionsResult)
      { this.dropPartitionsResult = dropPartitionsResult; return this; }

      TryDropPartitionsState exception(TException exception)
      { this.exception = exception; return this; }

      TryDropPartitionsState success(boolean success)
      { this.success = success; return this; }
    }

    private TryDropPartitionsState try_drop_partitions_req(RawStore ms, DropPartitionsRequest request) {
      ...
      try {
        ...
        ms.openTransaction();
        boolean isTableExternal = isExternal(tbl);
        startPartitionListFunction("drop_partitions", dbName, tblName, parts);
        for (Partition part : parts) {
        ...
        success = ms.commitTransaction();
        dropPartitionsResult = new DropPartitionsResult();
        if (needResult) {
          dropPartitionsResult.setPartitions(parts);
        }
      }
      ...
      return new TryDropPartitionsState()
          .envContext(envContext)
          .table(tbl)
          .parts(parts)
          .dirsToDelete(dirsToDelete)
          .archToDelete(archToDelete)
    ...
    // drop_partitions_req_epilogue() must always be called if try_drop_partitions_req() is called...
    // ... because although part-dirs are deleted only on success, notifications are sent unconditionally.
    private void drop_partitions_req_epilogue(TryDropPartitionsState stateFromDropPartitions) throws MetaException {
      boolean success = stateFromDropPartitions.success;
      boolean deleteData = stateFromDropPartitions.deleteData;
      boolean mustPurge = stateFromDropPartitions.mustPurge;
      Table tbl = stateFromDropPartitions.table;
      List<Partition> parts = stateFromDropPartitions.parts;
      List<Path> archToDelete = stateFromDropPartitions.archToDelete;
      List<PathAndPartValSize> dirsToDelete = stateFromDropPartitions.dirsToDelete;

      if (success && deleteData && tbl != null && !isExternal(tbl)) {
      ...
      if (parts != null) {
        for (Partition part : parts) {
          for (MetaStoreEventListener listener : listeners) {
            DropPartitionEvent dropPartitionEvent =
                new DropPartitionEvent(tbl, part, stateFromDropPartitions.success, deleteData, this);
            dropPartitionEvent.setEnvironmentContext(stateFromDropPartitions.envContext);
            listener.onDropPartition(dropPartitionEvent);
          }
        }
      }
    ...
    private static class TryAddPartitionsCoreState {
      // State. Might need cleanup.
      Table tbl;
      Map<PartValEqWrapper, Boolean> addedPartitions;
      List<Partition> parts;
      List<Partition> existingParts;
      ...
      TryAddPartitionsCoreState tbl(Table tbl)
      { this.tbl = tbl; return this; }

      TryAddPartitionsCoreState addedPartitions(Map<PartValEqWrapper, Boolean> addedPartitions)
      {this.addedPartitions = addedPartitions; return this;}

      TryAddPartitionsCoreState parts(List<Partition> parts)
      { this.parts = parts; return this; }

      TryAddPartitionsCoreState existingParts(List<Partition> existingParts)
      {this.existingParts = existingParts; return this;}

      TryAddPartitionsCoreState success(boolean success)
      { this.success = success; return this; }

      TryAddPartitionsCoreState exception(TException exception)
      { this.exception = exception; return this; }

      TryAddPartitionsCoreState result(List<Partition> result)
      { this.result = result; return this; }

    }

    private TryAddPartitionsCoreState try_add_partitions_core(
        RawStore ms,
        String dbName,
        String tblName,
        List<Partition> parts,
      ...
      logInfo("try_add_partitions_core");
      boolean success = false;
      TException exception = null;
      // Ensures that the list doesn't have dups, and keeps track of directories we have created.
      ...
      return new TryAddPartitionsCoreState()
                      .tbl(tbl)
    ...
    // Epilogue must always be called if try_add_partitions_core() is called...
    // ... because although cleanup is only required on failure, add_partitions notifications are sent unconditionally.
    private void add_partitions_core_epilogue(TryAddPartitionsCoreState tryAddPartitionsCoreState) throws TException {

      boolean success = tryAddPartitionsCoreState.success;
      Table tbl = tryAddPartitionsCoreState.tbl;
      List<Partition> parts = tryAddPartitionsCoreState.parts;
      List<Partition> existingParts = tryAddPartitionsCoreState.existingParts;
      List<Partition> result = tryAddPartitionsCoreState.result;
      Map<PartValEqWrapper, Boolean> addedPartitions = tryAddPartitionsCoreState.addedPartitions;
    ...
    public boolean drop_partition_with_environment_context(final String db_name,
      ...
      try {
        ...
        LOG.error("Exception performing operation, rethrowing", e);
        ex = e;
        throw new MetaException(e.getMessage());
      } catch (Exception e) {
        ex = e;
    ...
    private void rename_partition(final String db_name, final String tbl_name,
        ...
        throws InvalidOperationException, MetaException,
        TException {
      startPartitionFunction("alter_partition", db_name, tbl_name, part_vals);
    ...
    public void alter_partitions_with_environment_context(final String db_name, final String tbl_name,
      ...
      startPartitionListFunction("alter_partitions", db_name, tbl_name, new_parts);
    ...
    public List<String> get_all_tables(final String dbname) throws MetaException {
     startDatabaseFunction("get_all_tables", dbname);
    ...
    public List<FieldSchema> get_fields_with_environment_context(String db, String tableName,
        final EnvironmentContext envContext)
        throws MetaException, UnknownTableException, UnknownDBException {
      startTableFunction("get_fields_with_environment_context", db, tableName);
    ...
    public List<FieldSchema> get_schema_with_environment_context(String db, String tableName,
          final EnvironmentContext envContext)
        throws MetaException, UnknownTableException, UnknownDBException {
      startTableFunction("get_schema_with_environment_context", db, tableName);
      boolean success = false;
      Exception ex = null;
      try {
        String[] names = tableName.split("\\.");
    ...
    public Partition get_partition_by_name(final String db_name, final String tbl_name,
      ...
      startPartitionFunction("get_partition_by_name", db_name, tbl_name, part_name);
      Partition ret = null;
      Exception ex = null;
      try {
    ...
    public Partition append_partition_by_name_with_environment_context(final String db_name,
        ...
        throws InvalidObjectException, AlreadyExistsException, MetaException, TException {
      startPartitionFunction("append_partition_by_name", db_name, tbl_name, part_name);
    ...
    public boolean drop_partition_by_name_with_environment_context(final String db_name,
        ...
        MetaException, TException {
      startPartitionFunction("drop_partition_by_name", db_name, tbl_name, part_name);
      ...
      try {
        ...
        if (idxTblName != null) {
          ...
          partPaths = dropPartitionsAndGetLocations(ms, qualified[0], qualified[1], tblPath,
              tbl.getPartitionKeys(), false, deleteData);
      ...
      InvalidInputException, InvalidObjectException
    {
      ...
      startColumnFunction("get_column_statistics_by_table", dbName, tableName, colName);
    ...
    public TableStatsResult get_table_statistics_req(TableStatsRequest request)
      ...
      startTableFunction("get_table_statistics_req", dbName, tblName);
    ...
    public ColumnStatistics get_partition_column_statistics(String dbName, String tableName,
      ...
      startPartitionColumnFunction("get_column_statistics_by_partition", dbName, tableName, convertedPartName, colName);
      ColumnStatistics statsObj = null;
    ...
    public PartitionsStatsResult get_partitions_statistics_req(PartitionsStatsRequest request)
      ...
      String dbName = request.getDbName().toLowerCase();
      String tblName = request.getTblName().toLowerCase();
      startTableFunction("get_partitions_statistics_req", dbName, tblName);
      ...
      InvalidInputException
    {
      ...
      for (ColumnStatisticsObj statsObj:statsObjs) {
        ...
        startColumnFunction("write_column_statistics", dbName, tableName, colName);
      }
      ...
      InvalidInputException
    {
      ...
      for (ColumnStatisticsObj statsObj:statsObjs) {
        colName = statsObj.getColName().toLowerCase();
        statsObj.setColName(colName);
        startPartitionColumnFunction("write_partition_column_statistics", dbName, tableName, partName, colName);
      }
      ...
      InvalidObjectException, TException, InvalidInputException
    {
      ...
      String convertedPartName = lowerCaseConvertPartName(partName);
      startPartitionColumnFunction("delete_column_statistics_by_partition", dbName, tableName, convertedPartName, colName);
      boolean ret = false;
      ...
      InvalidInputException
   {
      ...
      startColumnFunction("delete_column_statistics_by_table", dbName, tableName, colName);
    ...
    public List<Partition> get_partitions_by_filter(final String dbName,
        ...
        throws MetaException, NoSuchObjectException, TException {
      startTableFunction("get_partitions_by_filter", dbName, tblName, filter);
    ...
    public Function get_function(String dbName, String funcName)
        throws MetaException, NoSuchObjectException, TException {
      startFunction("get_function", " db=" + dbName + " pat=" + funcName);
    ...
    public AggrStats get_aggr_stats_for(PartitionsStatsRequest request)
      ...
      String dbName = request.getDbName().toLowerCase();
      String tblName = request.getTblName().toLowerCase();
      startTableFunction("get_aggr_stats_for", request.getDbName(), request.getTblName());
    ...
    public ClearFileMetadataResult clear_file_metadata(ClearFileMetadataRequest req)
        throws TException {
      LOG.warn("clear_file_metadata() not supported!");
    ...
    public CacheFileMetadataResult cache_file_metadata(
      ...
      LOG.warn("cache_file_metadata() not supported!");
    ...
    try {
      ...
      if (useSasl) {
        ...
        transFactory = saslServer.createTransportFactory(
                MetaStoreUtils.getMetaStoreSaslProperties(conf), conf);
