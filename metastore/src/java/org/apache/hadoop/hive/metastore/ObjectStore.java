...
public class ObjectStore implements RawStore, Configurable {
  ...
  */
  ...
  private int maxPartitionsPermitted = -1;
  ...
  public void setConf(Configuration conf) {
    ...
    try {
      ...
      maxPartitionsPermitted =
          hiveConf.getInt(HiveConf.ConfVars.HIVELIMITTABLESCANPARTITION.varname, -1);
  ...
  // return false if any partitions protect mode is offline/no-drop/readonly.
  public boolean canDropAllPartitions(String dbName, String tblName, boolean allowsql)
      throws MetaException, NoSuchObjectException {
    boolean doUseDirectSql = allowsql && HiveConf.getBoolVar(getConf(), ConfVars.METASTORE_TRY_DIRECT_SQL)
      && HiveConf.getBoolVar(getConf(), ConfVars.METASTORE_TRY_DIRECT_SQL_DDL);

    if (doUseDirectSql) {
      directSql.canDropPartitions(dbName, tblName);
    } else {

      int partitionBatchSize = HiveConf.getIntVar(hiveConf,
          ConfVars.METASTORE_BATCH_RETRIEVE_TABLE_PARTITION_MAX);
      List<String> partitionNames = listPartitionNames(dbName, tblName, (short)-1);
      ...
      for(int i=0; i < partitionNames.size(); i+= partitionBatchSize) {
        List<String> partNames = partitionNames.subList(i, Math.min(i+partitionBatchSize,
            partitionNames.size()));
        List<Partition> listPartitions = getPartitionsByNames(dbName,tblName, partNames);
        for (Partition p: listPartitions) {
          String protectValue = p.getParameters().get(ProtectMode.PARAMETER_NAME);
          if (protectValue == null) {
            continue;
          }
          ProtectMode mode = ProtectMode.getProtectModeFromString(protectValue);
          if (mode.noDrop || mode.offline || mode.readOnly) {
            Table tbl = getTable(dbName, tblName);
            String partName = "";
            if (tbl != null) {
              partName = Warehouse.makePartName(tbl.getPartitionKeys(), p.getValues());
            }
  ...
  public void dropPartitions(String dbName, String tblName, final List<String> partNames)
    ...
    boolean success = new GetHelper<Boolean> (dbName, tblName, true, true) {
      @Override
      protected Boolean getSqlResult(GetHelper<Boolean> ctx) throws MetaException {
        directSql.dropPartitionsByName(dbName, tblName, partNames);
        LOG.info("Number of partitions dropped via directSQL:" + (partNames == null ? 0 : partNames.size()));
        return true;
      }
      @Override
      protected Boolean getJdoResult(
          GetHelper<Boolean> ctx) throws MetaException {
        int batchSize = HiveConf.getIntVar(hiveConf,
            ConfVars.METASTORE_BATCH_RETRIEVE_TABLE_PARTITION_MAX);
        MetaStoreUtils.callInBatch(partNames, batchSize, new MetaStoreUtils.BatchFunc<String>() {
          ...
          public void call(List<String> list) throws MetaException {
            dropPartitionsInternal(dbName, tblName, list);
          }
          @Override
        ...
        LOG.info("Number of partitions dropped via ORM:" + (partNames == null ? 0 : partNames.size()));
        return true;
      }
      @Override
    ...
    if (!success) {
      throw new MetaException("Drop partitions failed in object store.");
    }
  }
  
  private void dropPartitionsInternal(String dbName, String tblName, List<String> partNames)
      throws MetaException {
    // Delete all things.
  ...
  protected List<Partition> getPartitionsInternal(
    ...
    return new GetListHelper<Partition>(dbName, tblName, allowSql, allowJdo) {
      ...
      protected List<Partition> getSqlResult(GetHelper<List<Partition>> ctx) throws MetaException {
        ...
        List<Partition> parts = directSql.getPartitions(dbName, tblName, max, maxPartitionsPermitted);
        LOG.info("Number of partitions obtained via directSQL: " + (parts == null ? 0 : parts.size()));
        return parts;
      }
      @Override
      protected List<Partition> getJdoResult(
          GetHelper<List<Partition>> ctx) throws MetaException, NoSuchObjectException {
        List<Partition> parts = convertToParts(listMPartitions(dbName, tblName, maxParts));
        LOG.info("Number of partitions obtained via ORM: " + (parts == null ? 0 : parts.size()));
        return parts;
      }
    }.run(false);
  }
  ...
  public List<Partition> getPartitionsWithAuth(String dbName, String tblName,
    ...
    List<Partition> parts = getPartitions(dbName, tblName, max);
...
*/
  ...
  private Collection getPartitionPsQueryResults(String dbName, String tableName,
    ...
    } else if (maxPartitionsPermitted != -1 && max_parts == 0) {
      q.setRange(0, maxPartitionsPermitted + 1);
    }
    ...
    Collection parts = (Collection) q.execute(dbName, tableName, partNameMatcher);
    if (maxPartitionsPermitted != -1 && max_parts >= 0 && parts.size() > maxPartitionsPermitted) {
      String message = "Partitions selected (" + parts.size() +
          ") maxes out the partition limit (" + maxPartitionsPermitted +
          "), specify a more restrictive partition predicate";
      LOG.error(message);
  ...
  private List<MPartition> listMPartitions(String dbName, String tableName,
    ...
    try {
      ...
      } else if (maxPartitionsPermitted != -1 && max == 0) {
        query.setRange(0, maxPartitionsPermitted + 1);
      }
      mparts = (List<MPartition>) query.execute(tableName, dbName);
      if (maxPartitionsPermitted != -1 && max >= 0 && mparts.size() > maxPartitionsPermitted) {
        String message = "Partitions selected (" + mparts.size() +
            ") maxes out the partition limit (" + maxPartitionsPermitted +
            "), specify a more restrictive partition predicate";
        LOG.error(message);
        throw new MetaException(message);
  ...
  protected List<Partition> getPartitionsByNamesInternal(String dbName, String tblName,
    ...
    return new GetListHelper<Partition>(dbName, tblName, allowSql, allowJdo) {
      @Override
      protected List<Partition> getSqlResult(GetHelper<List<Partition>> ctx) throws MetaException {
        // Hard coding to '0' so directSQL can throw an error on reaching partition limit
        List<Partition> parts = directSql.getPartitionsViaSqlFilter(dbName, tblName, partNames, 0, maxPartitionsPermitted);
        LOG.info("Number of partitions obtained via directSQL:" + (parts == null ? 0 : parts.size()));
        return parts;
      }
      @Override
      protected List<Partition> getJdoResult(
          GetHelper<List<Partition>> ctx) throws MetaException, NoSuchObjectException {
        List<Partition> parts = getPartitionsViaOrmFilter(dbName, tblName, partNames);
        LOG.info("Number of partitions obtained via ORM:" + (parts == null ? 0 : parts.size()));
        return parts;
      }
    }.run(false);
  ...
  protected boolean getPartitionsByExprInternal(String dbName, String tblName, final byte[] expr,
    ...
    result.addAll(new GetListHelper<Partition>(dbName, tblName, allowSql, allowJdo) {
      ...
      protected List<Partition> getSqlResult(GetHelper<List<Partition>> ctx) throws MetaException {
        ...
        if (exprTree != null) {
          result = directSql.getPartitionsViaSqlFilter(ctx.getTable(), exprTree, max, maxPartitionsPermitted);
          LOG.info("Number of partitions obtained via directSQL:" + (result == null ? 0 : result.size()));
        }
        if (result == null) {
          ...
          result = directSql.getPartitionsViaSqlFilter(dbName, tblName, partNames, max, maxPartitionsPermitted);
          LOG.info("Number of partitions obtained via directSQL partition names:" + (result == null ? 0 : result.size()));
        }
        return result;
      }
      ...
      protected List<Partition> getJdoResult(
        ...
        if (exprTree != null) {
          ...
          LOG.info("Number of partitions obtained via ORM:" + (result == null ? 0 : result.size()));
        }
        if (result == null) {
          ...
          LOG.info("Number of partitions obtained via ORM partition names:" + (result == null ? 0 : result.size()));
        }
        return result;
      }
    }.run(true));
  ...
  private List<Partition> getPartitionsViaOrmFilter(Table table, ExpressionTree tree,
    ...
    if (maxParts > 0) {
      // User specified a row limit, set it on the Query
      query.setRange(0, maxParts);
    } else if (maxPartitionsPermitted != -1 && maxParts == 0) {
      // If partition limit is set, lets only retrieve the max number of partitions
      // Retrieving anything more than that will cause unnecessary load on system.
      query.setRange(0, maxPartitionsPermitted + 1);
    }
    ...
    if (maxPartitionsPermitted != -1 && maxParts >= 0 && mparts.size() > maxPartitionsPermitted) {
      String message = "Partitions selected (" + mparts.size() +
          ") maxes out the partition limit (" + maxPartitionsPermitted +
          "), specify a more restrictive partition predicate";
      LOG.error(message);
  ...
  private List<Partition> getPartitionsViaOrmFilter(
    ...
    try {
      ...
      if (maxPartitionsPermitted != -1 && mparts.size() > maxPartitionsPermitted) {
        String message = "Partitions selected (" + mparts.size() +
          ") maxes out the partition limit (" + maxPartitionsPermitted +
          "), specify a more restrictive partition predicate";
        LOG.error(message);
  ...
  private abstract class GetHelper<T> {
    ...
    private void handleDirectSqlError(Exception ex) throws MetaException, NoSuchObjectException {
      if (ex instanceof MetaException && ex.getMessage().startsWith(MetaStoreDirectSql.MAXPARTLIMIT_MSGPREFIX)) {
        LOG.info("Looks like partition limit reached, will ignore and throw a MetaException", ex);
  ...
  protected List<Partition> getPartitionsByFilterInternal(String dbName, String tblName,
    ...
    return new GetListHelper<Partition>(dbName, tblName, allowSql, allowJdo) {
      ...
      protected List<Partition> getSqlResult(GetHelper<List<Partition>> ctx) throws MetaException {
        Integer max = (maxParts < 0) ? null : (int) maxParts;
        List<Partition> parts = directSql.getPartitionsViaSqlFilter(ctx.getTable(), tree, max, maxPartitionsPermitted);
        if (parts == null) {
          // Cannot push down SQL filter. The message has been logged internally.
          // This is not an error so don't roll back, just go to JDO.
          ctx.disableDirectSql();
        } else {
          LOG.info("Number of partitions obtained via directSQL: " + parts.size());
        }
        return parts;
      }
      @Override
      protected List<Partition> getJdoResult(
          GetHelper<List<Partition>> ctx) throws MetaException, NoSuchObjectException {
        List<Partition> parts = getPartitionsViaOrmFilter(ctx.getTable(), tree, maxParts, true);
        LOG.info("Number of partitions obtained via ORM:" + (parts == null ? 0 : parts.size()));
        return parts;
      }
    }.run(true);
  ...
  /**
   * Search table names for keywords and return Database and Table names. This first
   * attempts to use DirectSQL search (which should mostly succeed) and if not falls
   * back to ORM (which is limited in search).
   *
   * @param keywords - list of keywords to match Metastore
   *
   * @return - list of tables which match the keywords. An extract table parameter
   *           mentions what criteria did the keywords match.
   */
  @Override
  public List<Table> listTablesByQuery(String[] keywords, long maxResults) throws MetaException {

    List<Table> tables = new ArrayList<Table>();
    List<TableSearchResult> results = null;

    boolean doUseDirectSql = HiveConf.getBoolVar(getConf(), ConfVars.METASTORE_TRY_DIRECT_SQL);
    if (doUseDirectSql) {
      try {
        results = directSql.searchTables(keywords, maxResults);
      } catch (Exception e) {
        doUseDirectSql = false;
        LOG.error("DirectSQL failed, falling back to ORM", e);
      }
    }

    if (!doUseDirectSql) {
      results = searchTablesORM(keywords);
    }

    if (results != null) {
      LOG.info("Converting search results to Tables " + results.size());
      for (TableSearchResult result : results) {
        Table table = getTable(result.getDatabase(), result.getTable());
        addTagsToTableProperty(result, table);
        tables.add(table);
        if (maxResults > 0 && tables.size() == maxResults) {
          LOG.info("Reached maxResults limit of " + maxResults);
          break;
        }
      }
    } else {
    ...
    LOG.info("Search results size:" + tables.size());
    return tables;
  }

  private void addTagsToTableProperty(TableSearchResult result, Table table) {
    if (table.getParameters() != null) {
      table.getParameters().put(TableSearchResult.HIVE_METASTORE_SEARCH_TAGS,
          result.getTagsAsString());
    } else {
      Map<String, String> params = new HashMap<String, String>();
      params.put(TableSearchResult.HIVE_METASTORE_SEARCH_TAGS,
          result.getTagsAsString());
  ...
  /**
   * This is a ORM equivalent of Table search. This is a less complex
   * implementation and does not search everything done in DirectSQL implementation
   * to make the code simple. This just resides as a fallback mechanism and is not
   * meant to be a replacement for the DirectSQL implementation.
   *
   * @param keywords List of keywords to match metastore
   *
   * @return List of table search objects which match keywords
   */
  private List<TableSearchResult> searchTablesORM(String[] keywords) throws MetaException {
    List<TableSearchResult> searchResults = new ArrayList<TableSearchResult>();
    String[] expressions = new String[] { " && " , " || " };

    for (String expr : expressions) {
      searchTable(searchResults, keywords, expr, "tableName", TableSearchResult.TABLE_NAME_MATCH);
      searchTable(searchResults, keywords, expr, "owner", TableSearchResult.OWNER_MATCH);
      searchTable(searchResults, keywords, expr, "sd.location", TableSearchResult.LOCATION_MATCH);
    }
    return searchResults;
  }

  private void searchTable(List<TableSearchResult> searchResults,
      String[] keywords, String expression, String colToBeMatched, String tag)
      throws MetaException {

    Query query = pm.newQuery(MTable.class);
    query.declareImports("import java.lang.String");
    query.setResult("database.name, tableName");
    query.setFilter(getFilterForTableSearchORM(keywords, expression, colToBeMatched));
    Object result = query.execute();
    MetaStoreUtils.addResults(searchResults, result, tag);
    query.closeAll();
  }

  private String getFilterForTableSearchORM(String[] keywords, String expression,
                                            String colToBeMatched) {

    StringBuilder filter = new StringBuilder();
    for (String keyword : keywords) {
      filter.append(colToBeMatched).append(".matches(\".*").
  ...
  private void alterPartitionNoTxn(String dbname, String name, List<String> part_vals,
    ...
    Partition part = convertToPart(oldp);
    part.getValues();
  ...
  private void removeUnusedColumnDescriptor(MColumnDescriptor oldCD) {
    ...
    try {
      ...
      Query query = pm.newQuery("select count(1) from org.apache.hadoop.hive.metastore.model.MStorageDescriptor"
          + "  where (this.cd == inCD)");
  ...
  */
  ...
  @Override
  public List<Path> dropAllPartitionsNoTxn(String dbname, String name, boolean checkLocation, boolean allowsql, Path tblPath) throws MetaException,
      NoSuchObjectException, InvalidObjectException, InvalidInputException {
    ...
    if (doUseDirectSql){
      if (checkLocation) {
        partPaths = directSql.getAlteredPartitionPaths(dbname, name);
      }
      directSql.dropAllPartitions(dbname, name);
    } else {
      ...
      while (true) {
        List<Partition> partsToDelete = getPartitions(dbname, name, partitionBatchSize);
        ...
        for (Partition part : partsToDelete) {
          if (checkLocation && part.getSd() != null &&
            ...
            if (tblPath == null ||
                (partPath != null && !HMSHandler.isSubdirectory(tblPath, partPath))) {
              partPaths.add(partPath);
            }
          }
          dropPartition(dbname, name, part.getValues());
        }
      }
    }
    return partPaths;
