...
public final class FileUtils {
   ...
   * @param conf hive configuration
   * @param skipTrash Whether (if true) to delete pre-existing data completely, or to move it to .Trash.
   * @return true if deletion successful
   * @throws FileNotFoundException
   * @throws IOException
   */
  public static boolean trashFilesUnderDir(FileSystem fs, Path f, Configuration conf, boolean skipTrash)
      throws IOException {
    FileStatus[] statuses = fs.listStatus(f, HIDDEN_FILES_PATH_FILTER);
    boolean result = true;
    for (FileStatus status : statuses) {
      result = result & moveToTrash(fs, status.getPath(), conf, skipTrash);
    }
    return result;
  }
  ...
  public static boolean trashFilesUnderDir(FileSystem fs, Path f, Configuration conf)
      throws IOException {
    return trashFilesUnderDir(fs, f, conf, false);
  }
   ...
   * @throws IOException
   */
  public static boolean moveToTrash(FileSystem fs, Path f, Configuration conf, boolean skipTrash) throws IOException {
    LOG.info("deleting  " + f + " with skipTrash == " + skipTrash);
    ...
    if (!skipTrash) {
      if (hadoopShim.moveToAppropriateTrash(fs, f, conf)) {
        ...
        LOG.error("Failed to move to trash: " + f + ". Attempting deletion.");
      }
    }
    else {
      LOG.info("Skipping moving " + f + " to trash. Attempting to delete completely.");
    }
  ...
  public static boolean moveToTrash(FileSystem fs, Path f, Configuration conf) throws IOException {
    return moveToTrash(fs, f, conf, false);
  }
