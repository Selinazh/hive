...
public class Metrics {
  ...
  public static class MetricsScope {
    ...
    /**
     * Resets all metrics associated for a scope to 0, could be used to initialize scope
     * @throws IOException
     */
    public void resetMetrics() throws IOException {
      synchronized (metrics) {
        Metrics.set(numCounter, new Long(0));
        Metrics.set(timeCounter, new Long(0));
        Metrics.set(avgTimeCounter, 0.0);
      }
    }
  ...
  /**
   * Initialize all metrics associated for a scope to 0
   * @param name
   */
  public static void initializeScope(String name) throws IOException {
    new MetricsScope(name).resetMetrics();
  }
