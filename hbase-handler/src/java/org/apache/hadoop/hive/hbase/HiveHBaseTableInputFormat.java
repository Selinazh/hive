...
public class HiveHBaseTableInputFormat extends TableInputFormatBase
  ...
  private Scan createFilterScan(JobConf jobConf, int iKey, int iTimestamp, boolean isKeyBinary)
    ...
    String columnString = jobConf.get(serdeConstants.LIST_COLUMNS);
    String columnTypeString = jobConf.get(serdeConstants.LIST_COLUMN_TYPES);
    String filterExprSerialized = jobConf.get(TableScanDesc.FILTER_EXPR_CONF_STR);
    if (filterExprSerialized == null || StringUtils.isBlank(columnString) || StringUtils.isBlank(columnTypeString)) {
      LOG.warn("Abandoning filter push-down for HBase table.");
      return scan;
    }
    ...
    String keyColName = columnString.split(",")[iKey];
    String colType = columnTypeString.split(",")[iKey];
