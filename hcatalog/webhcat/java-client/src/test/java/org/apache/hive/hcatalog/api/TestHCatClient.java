...
public class TestHCatClient {
  ...
  public static class RunMS implements Runnable {
    ...
    public RunMS(String msPort) {
      ...
      this.args.add("hive.metastore.event.listeners=org.apache.hive.hcatalog.api.MockMetastoreEventListener,org.apache.hive.hcatalog.listener.DbNotificationListener");
      this.args.add("-v");
      this.args.add("-p");
  ...
  public void testGetMessageBusTopicName() throws Exception {
    try {
      ...
      assertNotSame("MessageBus topic-name shouldn't be settable in createTable()!", messageBusTopicName, client.getMessageBusTopicName(dbName, tableName));
      HCatTable table = client.getTable(dbName, tableName);
      table.getTblProps().put(HCatConstants.HCAT_MSGBUS_TOPIC_NAME, messageBusTopicName);
      client.updateTableSchema(dbName, tableName, table);
      assertEquals("MessageBus topic-name should have been settable in updateTableSchema()!",
  ...
  private static List<HCatFieldSchema> constructSchema(Map<String, Type> columnSchema) {
    return Lists.newArrayList(
        Iterators.transform(columnSchema.entrySet().iterator(),
            new Function<Map.Entry<String, Type>, HCatFieldSchema>() {
              @Override
              public HCatFieldSchema apply(Map.Entry<String, Type> input) {
                try {
                  return new HCatFieldSchema(input.getKey(), input.getValue(), "");
                } catch (Throwable t) {
  ...
  private HCatTable createDbAndTable(HCatClient client,
                                     String dbName,
                                     String tableName,
                                     Map<String, Type> columnSchema,
                                     Map<String, Type> partitionSchema) throws Exception {

    client.dropDatabase(dbName, true, HCatClient.DropDBMode.CASCADE);
    client.createDatabase(HCatCreateDBDesc.create(dbName).build());
    HCatTable table = new HCatTable(dbName, tableName)
        .cols(constructSchema(columnSchema))
        .partCols(constructSchema(partitionSchema));
  ...
  private HCatAddPartitionDesc createAddPartitionDesc(HCatTable table, Map<String, String> partitionSpec) throws Exception {
    return HCatAddPartitionDesc.create(
                                  new HCatPartition(table,
                                      partitionSpec,
                                      makePartLocation(table, partitionSpec)))
                                  .build();
  }

  private void addPartition(HCatClient client, HCatTable table, Map<String, String> partitionSpec) throws Exception {
    client.addPartition(createAddPartitionDesc(table, partitionSpec));
  }

  private void createInitialPartitions(HCatClient client, HCatTable table) throws Exception {
    addPartition(client, table, new HashMap<String, String>(){{ put("grid", "AA"); put("dt", "20160428"); }});
    addPartition(client, table, new HashMap<String, String>(){{ put("grid", "AA"); put("dt", "20160429"); }});
    addPartition(client, table, new HashMap<String, String>(){{ put("grid", "BB"); put("dt", "20160429"); }});
    addPartition(client, table, new HashMap<String, String>(){{ put("grid", "CC"); put("dt", "20160429"); }});
  }

  private void cleanupAllPartitionsQuietly(HCatClient client, HCatTable table) {
    try {
      for (final String partValue : Arrays.asList("AA", "BB", "CC", "ZZ")) {
        client.dropPartitions( table.getDbName(), table.getTableName(),
                               new HashMap<String, String>() {{ put("grid", partValue); }}, true);
      }
    }
    catch(Exception exception) {
  ...
  private void cleanupDatabaseQuietly(HCatClient client, String dbName) {
    try {
      if (client != null) {
        client.dropDatabase(dbName, true, HCatClient.DropDBMode.CASCADE);
      }
  ...
  /**
   * Test that the specified partitions can be added and dropped atomically.
   */
  private void testDropAndAddPartitions_positive(HCatClient client, HCatTable table) throws Exception {

    String dbName = table.getDbName();
    String tableName = table.getTableName();

    createInitialPartitions(client, table);

    try {
      // Simultaneously drop all [AA, BB], and add ZZ.
      client.dropAndAddPartitions(
          dbName,
          tableName,
          Arrays.<Map<String, String>>asList(
              new HashMap<String, String>() {{ put("grid", "AA"); }},
              new HashMap<String, String>() {{ put("grid", "BB"); }}
          ),
          false, // dropIfExists == false.
          true,  // deleteData == true.
          Arrays.asList(
              createAddPartitionDesc(table, new HashMap<String, String>() {{ put("grid", "ZZ"); put("dt", "20160430"); }}),
              createAddPartitionDesc(table, new HashMap<String, String>() {{ put("grid", "ZZ"); put("dt", "20160501"); }}),
              createAddPartitionDesc(table, new HashMap<String, String>() {{ put("grid", "ZZ"); put("dt", "20160502"); }})
          ),
          false // addIfNotExists == false.
      );

      // Check that AA and BB are dropped, and that ZZ have been added.
      assertFalse("There should be no partitions with grid=\"AA\"",
          client.partitionExists(dbName, tableName, new HashMap<String,String>(){{ put("grid", "AA"); }}));
      assertFalse("There should be no partitions with grid=\"BB\"",
          client.partitionExists(dbName, tableName, new HashMap<String,String>(){{ put("grid", "BB"); }}));
      assertEquals("There should be 3 partitions with grid=\"ZZ\"", 3,
          client.getPartitions(dbName, tableName, new HashMap<String, String>(){{ put("grid", "ZZ"); }}).size());
    }
    catch (Exception unexpectedException) {
      fail("Unexpected unexpectedException!" + unexpectedException.getMessage());
    }
    finally {
      cleanupAllPartitionsQuietly(client, table);
    }
  }

  /**
   * Test that no partitions are added if the partition-drop fails.
   */
  private void testDropAndAddPartitions_failedDrop(HCatClient client, HCatTable table) throws Exception {
    ...
    try {
      client.dropAndAddPartitions(
          ...
          Collections.<Map<String, String>>singletonList(
              new HashMap<String, String>() {{ put("grid", "NON_EXISTENT_VALUE"); }}
          ),
          false, // dropIfExists == false.
          true,  // deleteData == true.
          Arrays.asList(
      ...
      fail("Dropping a non-existent partition should have failed!");

    } catch (Exception expected) {

      assertFalse("Partitions with grid=\"ZZ\" shouldn't have been added!",
          client.partitionExists(dbName, tableName, new HashMap<String, String>(){{ put("grid", "ZZ"); }}));
  ...
  /**
   * Test that there is no failure if there are no partitions to be added. Must fall back to dropPartitions().
   */
  private void testDropAndAddPartitions_emptyAddPartitionsSet(HCatClient client, HCatTable table) throws Exception {
    ...
    try {
      // Simultaneously drop all [AA, BB], and add <nothing>.
      client.dropAndAddPartitions(
          dbName,
          tableName,
          Arrays.<Map<String, String>>asList(
          ...
          false, // dropIfExists == false.
          true,  // deleteData == true.
          Collections.<HCatAddPartitionDesc>emptyList(),
          false // addIfNotExists == false.
      );

      // Check that AA and BB are dropped.
      assertFalse("There should be no partitions with grid=\"AA\"",
          client.partitionExists(dbName, tableName, new HashMap<String,String>(){{ put("grid", "AA"); }}));
      assertFalse("There should be no partitions with grid=\"BB\"",
          client.partitionExists(dbName, tableName, new HashMap<String,String>(){{ put("grid", "BB"); }}));
  ...
  /**
   * Test that no partitions are dropped if partition-addition fails.
   */
  private void testDropAndAddPartitions_failedAdd(HCatClient client, HCatTable table) throws Exception {
    ...
    try {
      client.dropAndAddPartitions(
          ...
          Collections.singletonList(
              ...
              createAddPartitionDesc(table, new HashMap<String, String>() {{ put("grid", "CC"); put("dt", "20160429"); }})
          ),
          false // addIfNotExists == false.
      );

      fail("Adding a pre-existing partition should have failed!");

    } catch (Exception expected) {

      assertTrue("Partitions with grid=\"AA\" should not have been dropped!",
          client.partitionExists(dbName, tableName, new HashMap<String,String>(){{ put("grid", "AA"); }}));
      assertTrue("Partitions with grid=\"BB\" should not have been dropped!",
          client.partitionExists(dbName, tableName, new HashMap<String,String>(){{ put("grid", "BB"); }}));
    }
    finally {
      cleanupAllPartitionsQuietly(client, table);
  ...
  @Test
  public void testDropAndAddPartitions() throws Exception {
    final String dbName = "myDb";
    final String tableName = "myTable";
    ...
    try {
      ...
      HCatTable table = createDbAndTable(
          client, dbName, tableName,
          new HashMap<String, Type>() {{ put("data_col_1", Type.INT); put("data_col_2", Type.STRING); }}, // Immaterial.
          new HashMap<String, Type>() {{ put("grid", Type.STRING); put("dt",   Type.STRING); }}
      );

      testDropAndAddPartitions_positive(client, table);
      testDropAndAddPartitions_failedDrop(client, table);
      testDropAndAddPartitions_failedAdd(client, table);
      testDropAndAddPartitions_emptyAddPartitionsSet(client, table);
    }
    catch (Exception unexpected) {
      LOG.error("Unexpected exception!", unexpected);
      assertTrue("Unexpected exception! " + unexpected.getMessage(), false);
      ...
      cleanupDatabaseQuietly(client, dbName);
    }
  }

  @Test
  public void testDropPartitionsWithBadPartitionSpec() throws Exception {
    try {
      HCatClient client = HCatClient.create(new Configuration(hcatConf));
      final String dbName = "myDb";
      final String tableName = "myTable";
      ...
      Map<String, String> badPartitionSpec = new HashMap<String, String>();
      badPartitionSpec.put("non_partition_key", "2012_01_01");

      try {
        client.dropPartitions(dbName, tableName, badPartitionSpec, true);
        fail("Dropping partitions using a non-partition-key should cause an IllegalArgumentException.");
      }
      catch (IllegalArgumentException ignore) {
        LOG.info("Received exception as expected: " + ignore);
      }
  ...
  public void testTableSchemaPropagation() throws Exception {
    try {
      ...
      EnumSet<HCatTable.TableAttribute> ignoreTableProperties
          = EnumSet.copyOf(HCatTable.DEFAULT_COMPARISON_ATTRIBUTES);
      ignoreTableProperties.remove(HCatTable.TableAttribute.TABLE_PROPERTIES);
      ...
      assertEquals("Table after deserialization should have been identical to sourceTable.",
          sourceTable.diff(targetTable, ignoreTableProperties), // Ignore the differences in Table-properties.
          HCatTable.NO_DIFF);
