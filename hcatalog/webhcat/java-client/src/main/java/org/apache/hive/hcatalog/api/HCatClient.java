...
public abstract class HCatClient {
  ...
  /**
   * Checks where any partitions exist that satisfy the (possibly partial) partitionSpec.
   * @param dbName The database name.
   * @param tableName The table name.
   * @param partitionSpec The partition specification. (Need not include all partition keys.)
   * @return True if at least one partition exists that matches the partitionSpec, else false.
   * @throws HCatException
   */
  public abstract boolean partitionExists(String dbName, String tableName, Map<String, String> partitionSpec)
      throws HCatException;

  /**
   * Lists unique sets of values for the specified partition-keys.
   * @param dbName The database name.
   * @param tableName The table name.
   * @param requiredPartKeys List of partition-keys, for which unique values are to be returned.
   * @param maxReturnValues The maximum number of return-values.
   * @param filter Filter string, to specify additional constraints on the partition key/values.
   *               The string follows the format of a partition-key filter.
   *               (E.g. "((dt >= '20150101') AND (dt <= '20151231'))").
   * @return *Unsorted* list of Maps, each associated with a unique combination of partition key-values,
   *         associated with the specified partition-keys.
   * @throws HCatException
   */
  public abstract List<Map<String, String>> getPartitionKeyValues(String dbName, String tableName,
                                                                  List<String> requiredPartKeys, int maxReturnValues, String filter)
    throws HCatException;
  ...
  /**
   * Atomically drop a set of partitions *AND* add a list of partitions, to the same table.
   * The operation goes through if, and only if:
   * 1. All the partitions identified by the dropPartitionSpecs are dropped successfully.
   * 2. All the partitions specified in addPartitionSpecs could be added.
   * If either of the above operations fails, no partitions are added or dropped.
   * @param dbName The database name.
   * @param tableName The table name.
   * @param dropPartitionSpecs List of partition-specifications, identifying partitions to be dropped.
   * @param dropIfExists Hive returns an error if the partition specified does not exist, unless ifExists is set to true.
   * @param deleteData Whether to delete the underlying data.
   * @param addPartitionSpecs List of partition-specifications, for addition.
   * @param addIfNotExists Hive returns an error if the partitions being added already exist, unless addIfNotExists is true.
   * @throws HCatException
   */
  public abstract void dropAndAddPartitions(String dbName, String tableName,
   ...
   * @param ifExists Hive returns an error if the partition specified does not exist, unless ifExists is set to true.
   * @param deleteData Whether to delete the underlying data.
   * @param skipTrash Whether skipTrash flag should be used when deleting data.
   * @throws HCatException,ConnectionFailureException
   */
   public abstract void dropPartitions(String dbName, String tableName,
