...
public class HCatTable {
  ...
  // Properties that shouldn't be set already when creating a table.
  private static List<String> tblPropsBlackList
      = Lists.newArrayList(HCatConstants.HCAT_MSGBUS_TOPIC_NAME.toLowerCase(),
                           hive_metastoreConstants.DDL_TIME.toLowerCase(),
                           "external");
  ...
  Table toHiveTable() throws HCatException {
    return toHiveTable(false); // Retain default behaviour.
  }

  Table toHiveTable(boolean forTableCreation) throws HCatException {
    Table newTable = new Table();
    newTable.setDbName(dbName);
    newTable.setTableName(tableName);
    if (tblProps != null) {
      newTable.setParameters(
          forTableCreation?
              filterBlackListedProperties(tblProps)
            : tblProps
      );
    }
  ...
  public HCatTable fileFormat(String fileFormat) {
    ...
    if (fileFormat.equalsIgnoreCase("rcfile")) {
      ...
      serdeLib(ColumnarSerDe.class.getName());
    }
    else
  ...
  public EnumSet<TableAttribute> diff(HCatTable rhs, EnumSet<TableAttribute> attributesToCheck) {
    ...
    for (TableAttribute attribute : attributesToCheck) {
      ...
      if (attribute.equals(TableAttribute.TABLE_PROPERTIES)) {
        if (!tablePropertiesAreEquivalent(tblProps, rhs.tblProps)) {
  ...
  private static boolean tablePropertiesAreEquivalent(Map<String, String> lhs, Map<String, String> rhs) {
    return equivalent(filterBlackListedProperties(lhs), filterBlackListedProperties(rhs));
  }
  ...
  private static Map<String, String> filterBlackListedProperties(Map<String, String> map) {
    return Maps.newHashMap(Maps.filterKeys(map, new Predicate<String>(){
      @Override
      public boolean apply(String input) {
        return !tblPropsBlackList.contains(input.toLowerCase());
      }
    }));
  }
  ...
  public HCatTable resolve(HCatTable rhs, EnumSet<TableAttribute> attributes) throws HCatException {
    ...
    for (TableAttribute attribute : attributes) {
      ...
      if (attribute.equals(TableAttribute.TABLE_PROPERTIES)) {
        tblProps.putAll(filterBlackListedProperties(rhs.tblProps));
      }
    }
