...
public class HCatInputFormat extends HCatBaseInputFormat {
  ...
  public HCatInputFormat(InputJobInfo inputJobInfo) {
    super(inputJobInfo);
  }
  ...
  public static HCatInputFormat setInput(
    ...
    throws IOException {
    return setInput(job, dbName, tableName, null);
  }
  ...
  public static HCatInputFormat setInput(
    ...
    Preconditions.checkNotNull(job, "required argument 'conf' is null");
    Preconditions.checkNotNull(tableName, "required argument 'tableName' is null");

    HCatInputFormat hCatInputFormat = setInput(job.getConfiguration(), dbName, tableName, filter);
    InputJobInfo inputJobInfo = (InputJobInfo) HCatUtil.deserialize(job.getConfiguration().get(HCatConstants.HCAT_KEY_JOB_INFO));
    List<Path> partitionPaths = Lists.newArrayListWithCapacity(inputJobInfo.getPartitions().size());
    for (PartInfo partInfo : inputJobInfo.getPartitions()) {
      partitionPaths.add(new Path(partInfo.getLocation()));
    }
    HCatUtil.getTokensForFileSystems(job.getCredentials(), partitionPaths, job.getConfiguration());
   ...
   * See {@link #setInput(org.apache.hadoop.conf.Configuration, String, String, String)}
   *
   * This call is now deprecated because without a link to the Job, the FS/storage credentials can't be fetched.
   * @deprecated Use {@link #setInput(Job, String, String)} instead.
   */
  @Deprecated
  public static HCatInputFormat setInput(
   ...
   *
   * This call is now deprecated because without a link to the Job, the FS/storage credentials can't be fetched.
   * @deprecated Use {@link #setInput(Job, String, String, String)} instead. This method will be removed in Hive 1.2.2.x.
   */
  @Deprecated
  public static HCatInputFormat setInput(
