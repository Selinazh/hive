...
class HiveClientCache {
  ...
  private ScheduledFuture<?> createCleanupThread(long interval) {
    ...
    // Create a daemon thread to allow Tez AM to shutdown without waiting for a user thread to complete.
  ...
  public IMetaStoreClient get(final HiveConf hiveConf) throws MetaException, IOException, LoginException {
    ...
    do {
      ...
      // are synchronized.
      synchronized (CACHE_TEARDOWN_LOCK) {
        cacheableHiveMetaStoreClient = hiveCache.getIfPresent(cacheKey);

        // TODO: This check may not be necessary as we can return the client from getOrCreate and let it reconnect
        // which should be no worse than spinning in this while loop. Should also be okay to return a client even
        // if it is no longer in the cache.
        if (cacheableHiveMetaStoreClient != null) {
          if (LOG.isDebugEnabled()) {
            LOG.debug("Acquiring client: " + Integer.toHexString(System.identityHashCode(cacheableHiveMetaStoreClient)));
          }

          cacheableHiveMetaStoreClient.acquire();
          return cacheableHiveMetaStoreClient;
        } else {
          LOG.warn("Cached client was evicted during initialization.");
        }
      }
    } while (cacheableHiveMetaStoreClient == null);

    // will not get here any way
    throw new IOException("Error getting hiveMetaStoreClient");
  }
