...
public class HCatUtil {
  ...
  private static final HashMap<String, String> hiveConfDefaults;
  private static volatile HiveClientCache hiveClientCache;

  static {
    // Load all of the default confing values from HiveConf.
    hiveConfDefaults = new HashMap<String, String>(HiveConf.ConfVars.values().length);
    for (HiveConf.ConfVars var : HiveConf.ConfVars.values()) {
      hiveConfDefaults.put(var.toString(), var.getDefaultValue());
    }
  }
  ...
  public static void getTokensForFileSystems(Credentials cred, Iterable<Path> paths, Configuration conf) throws IOException {
    if (UserGroupInformation.isSecurityEnabled()) {
      Set<FileSystem> fileSystems = Sets.newHashSet();
      for (Path path : paths) {
        fileSystems.add(path.getFileSystem(conf));
      }
      for (FileSystem fs : fileSystems) {
        final Token<?> tokens[] = fs.addDelegationTokens(getRenewerPrincipal(conf), cred);
  ...
  private static String getRenewerPrincipal(Configuration conf) throws IOException {
    String masterPrincipal = Master.getMasterPrincipal(conf);
    if (StringUtils.isBlank(masterPrincipal)) {
      throw new IOException("Cannot retrieve Master principal to use as token renewer.");
    }
   ...
   * then FosterStorageHandler is used to encapsulate the InputFormat, OutputFormat and SerDe.
   * This StorageHandler assumes the other supplied storage artifacts are for a file-based storage system.
   * @param hiveConf HiveConf used to configure the Configurable StorageHandler
   * @param storerInfo StorerInfo to definining the StorageHandler and InputFormat, OutputFormat and SerDe
   * @return storageHandler instance
   * @throws IOException
   */
  public static HiveStorageHandler getStorageHandler(HiveConf hiveConf, StorerInfo storerInfo) throws IOException {
    return getStorageHandler(hiveConf,
      storerInfo.getStorageHandlerClass(),
      storerInfo.getSerdeClass(),
      storerInfo.getIfClass(),
      storerInfo.getOfClass());
  ...
  public static HiveStorageHandler getStorageHandler(HiveConf hiveConf, PartInfo partitionInfo) throws IOException {
    return HCatUtil.getStorageHandler(
      hiveConf,
      partitionInfo.getStorageHandlerClassName(),
      partitionInfo.getSerdeClassName(),
   ...
   * This StorageHandler assumes the other supplied storage artifacts are for a file-based storage system.
   * @param hiveConf HiveConf that will be used to configure the Configurable StorageHandler
   ...
   * @throws IOException
   */
  public static HiveStorageHandler getStorageHandler(HiveConf hiveConf,
    ...
    if ((storageHandler == null) || (storageHandler.equals(FosterStorageHandler.class.getName()))) {
      try {
        ...
        fosterStorageHandler.setConf(hiveConf);
  ...
  public static HiveStorageHandler getStorageHandler(Configuration conf,
                             ...
                             String outputFormat)
    throws IOException {
    return getStorageHandler(getHiveConf(conf), storageHandler, serDe, inputFormat, outputFormat);
  }
   ...
   * @param hiveConf the config settings as found in the hive-site.xml only.
   * @param jobConf the config settings used to launch the job.
   * @return the set difference between hiveConf and jobConf.
   */
  private static Properties getHiveSiteOverrides(Configuration hiveConf, Configuration jobConf) {
    // return (hiveConf - jobConf);
    Properties difference = new Properties();
    for (Map.Entry<String,String> keyValue : hiveConf) {
      String key = keyValue.getKey();
      String hiveSiteValue = keyValue.getValue();
      String configValue = jobConf.getRaw(key);

      if (configValue == null) {
        difference.put(key, hiveSiteValue);
      } else if (hiveConfDefaults.containsKey(key)) {
        // Necessary to compare against HiveConf defaults as hive-site.xml is not available on task nodes (like AM).
        if (! configValue.equals(hiveConfDefaults.get(key))) {
          difference.put(key, configValue);
        }
      }
    }
  ...
  /**
   * This returns whether the table should be appended to or not based on table
   * parameters, HCatStorer arguments and configuration.
   *
   * @param table
   * @param configuration
   * @return whether table should be appended to or not.
   */
  public static boolean isImmutable(Table table, Configuration configuration) {

    boolean isImmutable;

    // If table has immutable property set, just rely on that.
    if (table.getParameters().containsKey(hive_metastoreConstants.IS_IMMUTABLE)) {
      isImmutable = table.isImmutable();
      LOG.info("Table " + table.getTableName() + " has immutable set to:" + isImmutable);
      return isImmutable;

    } else {
      // If not, rely on the configuration value (disabled by default).
      isImmutable = configuration.getBoolean(HCatConstants.HCAT_TABLE_OUTPUT_IMMUTABLE,
          HCatConstants.HCAT_TABLE_OUTPUT_DEFAULT_IMMUTABLE);
      LOG.info("HCatalog immutable behavior is set to : " + isImmutable);
      return isImmutable;
    }
  }
}
