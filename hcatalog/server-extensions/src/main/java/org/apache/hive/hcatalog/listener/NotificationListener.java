...
public class NotificationListener extends MetaStoreEventListener {
  ...
  protected synchronized void testAndCreateConnection() {
    ...
    try {
      // A previous connection failure may have left a stale session and set conn to null.
      // Remove any existing session and recreate.
      session.remove();
      if (conn != null) {
  ...
  protected boolean isConnectionHealthy() {
    boolean status = true;
    try {
      LOG.info("Checking if connection is healthy");
      ...
      LOG.error("isConnectionHealthy returns false with the following error "+ t);
      status = false;
    }
    return status;
  }
