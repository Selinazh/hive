...
import org.apache.hive.service.cli.thrift.ThriftCLIMetricsProcessor;
...
public final class KerberosSaslHelper {
  ...
  private static class CLIServiceProcessorFactory extends TProcessorFactory {
    ...
    public TProcessor getProcessor(TTransport trans) {
      ...
      if (service.getHiveConf().getBoolVar(HiveConf.ConfVars.METASTORE_METRICS_ENABLED)) {
        sqlProcessor = new ThriftCLIMetricsProcessor<Iface>(service);
      } else {
        sqlProcessor = new TCLIService.Processor(service);
      }
      return service.getHiveConf().getBoolean("hive.server2.enable.doAs", false) ?
