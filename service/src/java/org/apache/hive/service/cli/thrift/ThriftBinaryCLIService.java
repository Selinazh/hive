...
import org.apache.hadoop.hive.common.metrics.HiveThriftMetricsEventHandler;
...
public class ThriftBinaryCLIService extends ThriftCLIService {
  ...
  public void run() {
    try {
      ...
      boolean tcpKeepAlive = hiveConf.getBoolVar(ConfVars.HIVE_SERVER2_TCP_KEEP_ALIVE);
      ...
      if (!hiveConf.getBoolVar(ConfVars.HIVE_SERVER2_USE_SSL)) {
        serverSocket = HiveAuthFactory.getServerSocket(hiveHost, portNum, tcpKeepAlive);
      } else {
      ...
      if(hiveConf.getBoolVar(ConfVars.METASTORE_METRICS_ENABLED)) {
        TServerEventHandler eventHandler = new HiveThriftMetricsEventHandler(HS_SERVICE_METRICS_PREFIX);
        serverEventHandler.addEventHandler(eventHandler);
      }
      serverEventHandler.addEventHandler(new RateLimitingEventHandler());
      serverEventHandler.addEventHandler(new DisconnectCleanupEventHandler(this, hiveConf));
