...
struct TResourceStatus {
  1: optional string resourceName
  2: optional string resourceType
  3: optional string usedCapacity
  4: optional string maxCapacity
}

struct TResourceStatusList {
  1: optional list<TResourceStatus> resourcesConsumed
}


// Contains details of a single operation - query/status/user/session info etc
// To be used by the AdminAPI only.
struct TOperationStatus {
  1: optional string user
  2: optional string host
  3: optional string sessionID
  4: optional string operationHandleID
  5: optional string tezSessionID
  6: optional TOperationState operationStatus
  7: optional TOperationType operationType
  8: optional string query
  9: optional string trackingURL
}

struct TActiveOperations {
  1: optional list<TOperationStatus> activeOperations;
}

struct TConfigurationEntry {
  1: required string paramName
  2: required string paramValue
}

struct TConfigurations {
  1: optional list<TConfigurationEntry> entries;
}
...
service TCLIService {
  ...
  TResourceStatusList GetResourceConsumptionList ();

  TActiveOperations GetActiveOperations ();

  TConfigurations GetConfigurations ();
